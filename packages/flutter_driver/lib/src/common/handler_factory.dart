...
mixin CreateFinderFactory {
  ...
  Finder createFinder(SerializableFinder finder) {
    return switch (finder.finderType) {
      ...
      'BySemanticsLabel' =>
        _createBySemanticsLabelFinder(finder as BySemanticsLabel),
      ...
      final String type =>
        throw DriverError('Unsupported search specification type $type'),
    };
  }
  ...
  Finder _createByValueKeyFinder(ByValueKey arguments) {
    return switch (arguments.keyValueType) {
      ...
      _ => throw UnimplementedError(
          'Unsupported ByValueKey type: ${arguments.keyValueType}'),
    };
  }
...
mixin CommandHandlerFactory {
  ...
  Future<Result> handleCommand(Command command, WidgetController prober,
    ...
    return switch (command.kind) {
      ...
      'waitUntilNoTransientCallbacks' =>
        _waitUntilNoTransientCallbacks(command),
      'waitUntilNoPendingFrame' => _waitUntilNoPendingFrame(command),
      'waitUntilFirstFrameRasterized' =>
        _waitUntilFirstFrameRasterized(command),
  ...
  Future<Health> _getHealth(Command command) async =>
  ...
  Future<Result> _sendTextInputAction(Command command) async {
    ...
    final SendTextInputAction sendTextInputAction =
        command as SendTextInputAction;
    _testTextInput.receiveAction(
  ...
  Future<Result> _tap(Command command, WidgetController prober,
      CreateFinderFactory finderFactory) async {
  ...
  Future<Result> _waitForAbsent(
    ...
    await waitForAbsentElement(
        finderFactory.createFinder(waitForAbsentCommand.finder));
    return Result.empty;
  }

  Future<Result> _waitForTappable(
      Command command, CreateFinderFactory finderFactory) async {
  ...
  Future<Result> _waitForCondition(Command command) async {
    final WaitForCondition waitForConditionCommand =
        command as WaitForCondition;
    final WaitCondition condition =
        deserializeCondition(waitForConditionCommand.condition);
  ...
  Future<GetSemanticsIdResult> _getSemanticsId(
    ...
    final Finder target = await waitForElement(
        finderFactory.createFinder(semanticsCommand.finder));
  ...
  Future<DiagnosticsTreeResult> _getDiagnosticsTree(
    ...
    final Element element = finder.evaluate().single;
    final DiagnosticsNode diagnosticsNode =
    ...
    return DiagnosticsTreeResult(
        diagnosticsNode.toJsonMap(DiagnosticsSerializationDelegate(
      subtreeDepth: diagnosticsCommand.subtreeDepth,
      includeProperties: diagnosticsCommand.includeProperties,
    )));
  }

  Future<Result> _scroll(Command command, WidgetController prober,
      CreateFinderFactory finderFactory) async {
    final Scroll scrollCommand = command as Scroll;
    final Finder target =
        await waitForElement(finderFactory.createFinder(scrollCommand.finder));
    final int totalMoves = scrollCommand.duration.inMicroseconds *
        scrollCommand.frequency ~/
        Duration.microsecondsPerSecond;
    final Offset delta =
        Offset(scrollCommand.dx, scrollCommand.dy) / totalMoves.toDouble();
  ...
  Future<GetTextResult> _getText(
      Command command, CreateFinderFactory finderFactory) async {
    final GetText getTextCommand = command as GetText;
    final Finder target =
        await waitForElement(finderFactory.createFinder(getTextCommand.finder));
    ...
    if (text == null) {
      throw UnsupportedError(
          'Type ${widget.runtimeType} is currently not supported by getText');
    }
  ...
  Future<Result> _setTextEntryEmulation(Command command) async {
    final SetTextEntryEmulation setTextEntryEmulationCommand =
