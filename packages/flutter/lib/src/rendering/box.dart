...
///  * [RenderBox.hitTest], which documents more details around hit testing
///    [RenderBox]es.
typedef BoxHitTestWithOutOfBandPosition = bool Function(
...
class BoxHitTestResult extends HitTestResult {
  ...
  bool addWithOutOfBandPosition({
    ...
    if (paintOffset != null) {
      ...
      paintTransform = Matrix4.tryInvert(
...
abstract class _CachedLayoutCalculation<Input extends Object, Output> {
  ...
  Output memoize(_LayoutCacheStorage cacheStorage, Input input,
  ...
  Map<String, String> debugFillTimelineArguments(
      Map<String, String> timelineArguments, Input input);
  String eventLabel(RenderBox renderBox);
}

final class _DryLayout
    implements _CachedLayoutCalculation<BoxConstraints, Size> {
  const _DryLayout();

  @override
  Size memoize(_LayoutCacheStorage cacheStorage, BoxConstraints input,
  ...
  @override
  Map<String, String> debugFillTimelineArguments(
      Map<String, String> timelineArguments, BoxConstraints input) {
...
final class _Baseline
    implements
        _CachedLayoutCalculation<(BoxConstraints, TextBaseline),
            BaselineOffset> {
  const _Baseline();

  @override
  BaselineOffset memoize(
      _LayoutCacheStorage cacheStorage,
      (BoxConstraints, TextBaseline) input,
      BaselineOffset Function((BoxConstraints, TextBaseline)) computer) {
    final Map<BoxConstraints, BaselineOffset> cache = switch (input.$2) {
      TextBaseline.alphabetic => cacheStorage._cachedAlphabeticBaseline ??=
          <BoxConstraints, BaselineOffset>{},
  ...
  @override
  Map<String, String> debugFillTimelineArguments(
      Map<String, String> timelineArguments,
      (BoxConstraints, TextBaseline) input) {
...
enum _IntrinsicDimension implements _CachedLayoutCalculation<double, double> {
  ...
  @override
  double memoize(_LayoutCacheStorage cacheStorage, double input,
      double Function(double) computer) {
    return (cacheStorage._cachedIntrinsicDimensions ??=
            <(_IntrinsicDimension, double), double>{})
        .putIfAbsent((this, input), () => computer(input));
  }

  @override
  Map<String, String> debugFillTimelineArguments(
      Map<String, String> timelineArguments, double input) {
    return timelineArguments
  ...
  @override
  String eventLabel(RenderBox renderBox) =>
      '${renderBox.runtimeType} intrinsics';
}
...
abstract class RenderBox extends RenderObject {
  ...
  Output _computeIntrinsics<Input extends Object, Output>(
    ...
    assert(RenderObject.debugCheckingIntrinsics ||
        !debugDoingThisResize); // performResize should not depend on anything except the incoming constraints
    ...
    return shouldCache
        ? _computeWithTimeline(type, input, computer)
        : computer(input);
  }
  ...
  Output _computeWithTimeline<Input extends Object, Output>(
    ...
    assert(() {
      ...
      debugTimelineArguments =
          type.debugFillTimelineArguments(arguments, input);
      return true;
    }());
    if (!kReleaseMode) {
      if (debugProfileLayoutsEnabled || _debugIntrinsicsDepth == 0) {
        FlutterTimeline.startSync(type.eventLabel(this),
  ...
  Size getDryLayout(covariant BoxConstraints constraints) {
    return _computeIntrinsics(
        _CachedLayoutCalculation.dryLayout, constraints, _computeDryLayout);
  }
  ...
  double? getDryBaseline(
    ...
    final double? baselineOffset = _computeIntrinsics(
  ...
  BaselineOffset _computeDryBaseline((BoxConstraints, TextBaseline) pair) {
    ...
    final BaselineOffset result =
        BaselineOffset(computeDryBaseline(pair.$1, pair.$2));
    assert(() {
      assert(_computingThisDryBaseline);
  ...
  Size get size {
    ...
    assert(() {
      ...
      if (size is _DebugSize) {
        ...
        // layout callback).
        final bool doingRegularLayout = !(RenderObject
                .debugActiveLayout?.debugDoingThisLayoutWithCallback ??
            true);
        ...
        final RenderBox? renderBoxDoingDryBaseline = _computingThisDryBaseline
            ? this
            : (parent is RenderBox && parent._computingThisDryBaseline
                ? parent
                : null);
        assert(
    ...
    return _size ??
        (throw StateError(
            'RenderBox was not laid out: $runtimeType#${shortHash(this)}'));
  }
    ...
    assert(RenderObject.debugCheckingIntrinsics ||
        switch (owner!) {
          ...
          PipelineOwner(debugDoingPaint: true) =>
            RenderObject.debugActivePaint == parent &&
    ...
    assert(() {
      ...
      if (!_size!.isFinite) {
        ...
        if (!constraints.hasBoundedWidth) {
          RenderBox node = this;
          while (
              !node.constraints.hasBoundedWidth && node.parent is RenderBox) {
      ...
      if (debugCheckIntrinsicSizes) {
        ...
        double testIntrinsic(double Function(double extent) function,
            String name, double constraint) {
        ...
        try {
          ...
          if (constraints.hasBoundedHeight) {
            testIntrinsicsForValues(getMinIntrinsicHeight,
                getMaxIntrinsicHeight, 'Height', constraints.maxWidth);
          }
        ...
        if (failures.isNotEmpty) {
          ...
          throw FlutterError.fromParts(<DiagnosticsNode>[
            ...
            ErrorDescription(
                'The following ${failures.length > 1 ? "failures" : "failure"} was detected:'), // should this be tagged as an error or not?
            ...failures,
            ErrorHint(
              'If you are not writing your own RenderBox subclass, then this is not\n'
              'your fault. Contact support: https://github.com/flutter/flutter/issues/new?template=2_bug.yml',
