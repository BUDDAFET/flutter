...
class RenderConstrainedBox extends RenderProxyBox {
  ...
  double? computeDryBaseline(
    ...
    return child?.getDryBaseline(
        _additionalConstraints.enforce(constraints), baseline);
  }
  ...
  Size _computeSize(
    ...
    if (child != null) {
      final Size childSize =
          layoutChild(child!, _limitConstraints(constraints));
...
class RenderAspectRatio extends RenderProxyBox {
  ...
  double? computeDryBaseline(
    ...
    return super.computeDryBaseline(
        BoxConstraints.tight(getDryLayout(constraints)), baseline);
  }
  ...
  void debugPaintSize(PaintingContext context, Offset offset) {
    assert(() {
      if (child != null) {
        ...
        if (clipBehavior != Clip.none) {
          context.canvas
              .drawPath(_getClipPath(_clip).shift(offset), _debugPaint!);
          _debugText!.paint(
              context.canvas,
              offset +
...
class RenderPhysicalModel extends _RenderPhysicalModelBase<RRect> {
  ...
  RRect get _defaultClip {
    ...
    return switch (_shape) {
      ...
      BoxShape.circle =>
        RRect.fromRectXY(rect, rect.width / 2, rect.height / 2),
    };
  }
  ...
  void _updatePaintData() {
    ...
    if (child == null) {
      ...
      final Rect sourceRect =
          resolvedAlignment.inscribe(sizes.source, Offset.zero & childSize);
      final Rect destinationRect =
          resolvedAlignment.inscribe(sizes.destination, Offset.zero & size);
