...
mixin ServicesBinding on BindingBase, SchedulerBinding {
  ...
  List<AppLifecycleState> _generateStateTransitions(
    ...
    if (previousState == null) {
      ...
      if (state == AppLifecycleState.detached) {
        for (int i = previousStateIndex + 1;
            i < AppLifecycleState.values.length;
            ++i) {
  ...
  static bool _debugVerifyLifecycleChange(
    ...
    return switch (starting) {
      ...
      AppLifecycleState.detached => ending == AppLifecycleState.resumed ||
          ending == AppLifecycleState.paused,
      AppLifecycleState.inactive => ending == AppLifecycleState.resumed ||
          ending == AppLifecycleState.hidden,
      AppLifecycleState.hidden => ending == AppLifecycleState.paused ||
          ending == AppLifecycleState.inactive,
      AppLifecycleState.paused => ending == AppLifecycleState.hidden ||
  ...
  Future<dynamic> _handlePlatformMessage(MethodCall methodCall) async {
    ...
    switch (method) {
      ...
      case 'ContextMenu.onDismissSystemContextMenu':
        for (final SystemContextMenuClient client
            in _systemContextMenuClients) {
  ...
  final Set<SystemContextMenuClient> _systemContextMenuClients =
  ...
  static void unregisterSystemContextMenuClient(
      SystemContextMenuClient client) {
