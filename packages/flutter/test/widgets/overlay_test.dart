...
void main() {
  ...
  testWidgets('debugVerifyInsertPosition', (WidgetTester tester) async {
    ...
    try {
      overlay.insert(
        ...
        above: _buildOverlayEntry(
          (BuildContext context) {
            return Container();
          },
        ),
        below: _buildOverlayEntry(
          (BuildContext context) {
            return Container();
          },
        ),
    ...
    try {
      overlay.rearrange(<OverlayEntry>[base], above: _buildOverlayEntry(
        (BuildContext context) {
          return Container();
        },
      ));
    ...
    await tester.pumpWidget(
      Directionality(
        ...
        child: Overlay(
          initialEntries: <OverlayEntry>[
            _buildOverlayEntry((BuildContext context) =>
                Positioned(left: 2000, right: 2500, child: Container())),
          ],
        ),
      ),
    );
  ...
  testWidgets('OverlayEntry throws if inserted to an invalid Overlay',
      (WidgetTester tester) async {
    await tester.pumpWidget(
    ...
    final OverlayEntry entry =
        OverlayEntry(builder: (BuildContext context) => const SizedBox());
    addTearDown(() => entry
      ..remove()
      ..dispose());
    expect(
  ...
  group('OverlayEntry listenable', () {
    ...
    testWidgets('throw if disposed before removal',
        (WidgetTester tester) async {
      await tester.pumpWidget(emptyOverlay);
      final OverlayState overlay = overlayKey.currentState! as OverlayState;
      final OverlayEntry entry = OverlayEntry(
  ...
  group('LookupBoundary', () {
    testWidgets('hides Overlay from Overlay.maybeOf',
        (WidgetTester tester) async {
    ...
    testWidgets('hides Overlay from debugCheckHasOverlay',
        (WidgetTester tester) async {
      late final OverlayEntry baseEntry;
      addTearDown(() => baseEntry
        ..remove()
  ...
  testWidgets('Overlay is sized by child in an unconstrained environment',
    ...
    await tester.pumpWidget(
      Directionality(
        textDirection: TextDirection.ltr,
        child: UnconstrainedBox(
          child: Overlay(initialEntries: <OverlayEntry>[initialEntry]),
        ),
      ),
    );
  ...
  testWidgets('Overlay throws if unconstrained and has no child',
      (WidgetTester tester) async {
