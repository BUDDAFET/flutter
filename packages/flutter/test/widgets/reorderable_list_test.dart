...
void main() {
  ...
  testWidgets('SliverReorderableList item has correct semantics',
      (WidgetTester tester) async {
  ...
  testWidgets('SliverReorderableList - custom proxyDecorator',
      (WidgetTester tester) async {
  ...
  testWidgets('SliverReorderableList properly disposes items',
      (WidgetTester tester) async {
            ...
            builder: (_, BoxConstraints view) {
              ...
              return Scaffold(
                body: CustomScrollView(
                  ...
                  cacheExtent:
                      0, // The fourth one will not be created in the initial state.
                  slivers: <Widget>[
                    SliverReorderableList(
                      itemBuilder: (BuildContext context, int index) {
                        return ReorderableDragStartListener(
  ...
  testWidgets(
    ...
    Future<void> testMove(int from, int to,
      ...
      final Offset moveOffset =
          targetPosition - tester.getTopLeft(find.text('$from'));
      await tester.timedDrag(
          find.text('$from'), moveOffset, const Duration(seconds: 1));
      ...
      await tester.pump(const Duration(milliseconds: 125));
      expect(tester.getTopLeft(find.text('$from')),
    ...
    for (final (int, int) element in testCases) {
      ...
      await testMove(element.$1, element.$2,
          reverse: true, scrollDirection: Axis.horizontal);
    }
  });

  testWidgets(
      'Tests that the item position is correct when prototypeItem or itemExtent are set',
      (WidgetTester tester) async {
    ...
    await testFor(
        prototypeItem:
            const SizedBox(height: 100, width: 100, child: Text('prototype')));
    await testFor(itemExtent: 100);
  });

  testWidgets(
      'The item being dragged will not be affected by layout constraints.',
      (WidgetTester tester) async {
    final Map<int, BoxConstraints> itemLayoutConstraints =
        <int, BoxConstraints>{};
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
    ...
    final Map<int, BoxConstraints> preDragLayoutConstraints =
        Map<int, BoxConstraints>.of(itemLayoutConstraints);
