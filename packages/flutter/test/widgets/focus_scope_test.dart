...
void main() {
  group('FocusScope', () {
    ...
    testWidgets('Setting parentNode determines focus tree hierarchy.',
        (WidgetTester tester) async {
    ...
    testWidgets('Setting parentNode determines focus scope tree hierarchy.',
      ...
      addTearDown(childNode.dispose);
      final FocusScopeNode insertedNode =
          FocusScopeNode(debugLabel: 'Inserted');
      addTearDown(insertedNode.dispose);
    ...
    testWidgets('Removing a FocusScope removes its node from the tree',
      ...
      expect(
          FocusManager.instance.rootScope.descendants, isNot(contains(aScope)));
      expect(
          FocusManager.instance.rootScope.descendants, isNot(contains(bScope)));
    });
    ...
    testWidgets('Can focus root node.', (WidgetTester tester) async {
      ...
      expect(rootNode,
          equals(FocusManager.instance.rootScope.descendants.toList()[1]));
    });
  ...
  group('Focus', () {
    testWidgets('Focus.of stops at the nearest Focus widget.',
      ...
      expect(Focus.of(element6).parent!.parent!.parent!.parent!.parent,
          equals(root));
    });
    ...
    testWidgets(
        'Focus widget gains input focus when it gains accessibility focus',
        (WidgetTester tester) async {
      final SemanticsTester semantics = SemanticsTester(tester);
