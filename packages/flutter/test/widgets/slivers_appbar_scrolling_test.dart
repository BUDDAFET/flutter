...
void main() {
        ...
        child: CustomScrollView(
          slivers: <Widget>[
            ...
            SliverPersistentHeader(
                key: key2 = GlobalKey(), delegate: TestDelegate()),
            SliverPersistentHeader(
                key: key3 = GlobalKey(), delegate: TestDelegate()),
  ...
  testWidgets('Sliver appbars - scrolling off screen',
    ...
    final ScrollPosition position =
        tester.state<ScrollableState>(find.byType(Scrollable)).position;
    position.animateTo(RenderBigSliver.height + delegate.maxExtent - 5.0,
        curve: Curves.linear, duration: const Duration(minutes: 1));
    await tester.pumpAndSettle(const Duration(milliseconds: 1000));
    final RenderBox box =
        tester.renderObject<RenderBox>(find.text('Sliver App Bar'));
  ...
  group('has correct semantics when', () {
    ...
    testWidgets('partially scrolling off screen', (WidgetTester tester) async {
      ...
      final ScrollPosition position =
          tester.state<ScrollableState>(find.byType(Scrollable)).position;
      position.animateTo(delegate.maxExtent - 20.0,
          curve: Curves.linear, duration: const Duration(minutes: 1));
      await tester.pumpAndSettle(const Duration(milliseconds: 1000));
      final RenderBox box =
          tester.renderObject<RenderBox>(find.text('Sliver App Bar'));
    ...
    testWidgets('completely scrolling off screen and not within cache extent',
      ...
      final ScrollPosition position =
          tester.state<ScrollableState>(find.byType(Scrollable)).position;
      position.animateTo(delegate.maxExtent + 300.0,
          curve: Curves.linear, duration: const Duration(minutes: 1));
      await tester.pumpAndSettle(const Duration(milliseconds: 1000));
