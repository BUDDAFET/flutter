...
void verifyActualBoxPosition(
    WidgetTester tester, Finder finder, int index, Rect ideal) {
  final RenderBox box =
      tester.renderObjectList<RenderBox>(finder).elementAt(index);
...
void main() {
  testWidgets("Sliver appbars - floating - scroll offset doesn't change",
    ...
    final double max = bigHeight * 2.0 +
  ...
  testWidgets('Sliver appbars - floating - normal behavior works',
      (WidgetTester tester) async {
        ...
        child: CustomScrollView(
          slivers: <Widget>[
            ...
            SliverPersistentHeader(
                key: key2 = GlobalKey(), delegate: delegate, floating: true),
    ...
    verifyActualBoxPosition(
        ...
        0,
        Rect.fromLTWH(
            0.0, 600.0 - delegate.maxExtent, 800.0, delegate.maxExtent));
    verifyPaintPosition(key3, const Offset(0.0, 600.0), false);

    assert(delegate.maxExtent * 2.0 <
        600.0); // make sure this fits on the test screen...
    position.animateTo(bigHeight - 600.0 + delegate.maxExtent * 2.0,
        curve: Curves.linear, duration: const Duration(minutes: 1));
    await tester.pumpAndSettle(const Duration(milliseconds: 1000));
    verifyPaintPosition(key1, Offset.zero, true);
    verifyPaintPosition(
        key2, Offset(0.0, 600.0 - delegate.maxExtent * 2.0), true);
    verifyActualBoxPosition(
        tester,
        find.byType(Container),
        0,
        Rect.fromLTWH(
            0.0, 600.0 - delegate.maxExtent * 2.0, 800.0, delegate.maxExtent));
    verifyPaintPosition(key3, Offset(0.0, 600.0 - delegate.maxExtent), true);

    position.animateTo(bigHeight,
        curve: Curves.linear, duration: const Duration(minutes: 1));
    await tester.pumpAndSettle(const Duration(milliseconds: 1000));
  ...
  testWidgets(
    ...
    final ScrollPositionWithSingleContext position = tester
  ...
  group('Pointer scrolled floating', () {
    ...
    testWidgets('SliverAppBar', (WidgetTester tester) async {
      ...
      await tester
          .sendEventToBinding(testPointer.scroll(const Offset(0.0, -250.0)));
      await tester.pump();
      expect(find.text('Test Title'), findsOneWidget);
      expect(find.text('Item 1'), findsOneWidget);
      expect(find.text('Item 5'), findsOneWidget);
