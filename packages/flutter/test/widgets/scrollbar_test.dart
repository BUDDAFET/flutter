...
To use the PrimaryScrollController explicitly, set ScrollView.primary to true on the Scrollable widget.''',
  ...
  testWidgets('Scrollbars assert on multiple scroll positions',
      (WidgetTester tester) async {
    final ScrollController scrollController = ScrollController();
    addTearDown(scrollController.dispose);
    await tester.pumpWidget(
    ...
    final TestGesture gesture =
        await tester.startGesture(tester.getCenter(scrollViews.first));
    await gesture.moveBy(const Offset(0.0, -20.0));
    exception = tester.takeException() as AssertionError;
    expect(exception, isAssertionError);
    expect(
...
The provided ScrollController cannot be shared by multiple ScrollView widgets.''',
  ...
  testWidgets('The bar can show or hide when the view size change',
      (WidgetTester tester) async {
  ...
  testWidgets('RawScrollbar correctly assigns colors',
      (WidgetTester tester) async {
    final ScrollController scrollController = ScrollController();
    addTearDown(scrollController.dispose);
    await tester.pumpWidget(
    ...
    Widget buildApp(bool buildBar) {
      return Directionality(
        ...
        child: MediaQuery(
          data: MediaQueryData(
            invertColors:
                buildBar, // Trigger a post frame check before unmount.
          ),
          child: PrimaryScrollController(
            controller: scrollController,
            child: LayoutBuilder(
  ...
  testWidgets('Track offset respects MediaQuery padding',
      (WidgetTester tester) async {
  ...
  testWidgets('RawScrollbar.padding replaces MediaQueryData.padding',
      (WidgetTester tester) async {
    final ScrollController scrollController = ScrollController();
    addTearDown(scrollController.dispose);
    await tester.pumpWidget(Directionality(
  ...
  testWidgets(
    ...
    final TestGesture dragScrollbarGesture =
        await tester.startGesture(const Offset(797.0, 5.0));
    await tester.pumpAndSettle();
    await dragScrollbarGesture.moveBy(const Offset(0.0, scrollAmount));
    await tester.pumpAndSettle();
  ...
  testWidgets('The bar supports mouse wheel event',
      (WidgetTester tester) async {
  ...
  testWidgets(
    ...
    await tester.flingFrom(
        const Offset(797.0, 45.0), const Offset(0, 60.0), 500.0);
    await tester.pumpAndSettle();

    if (isMobilePlatform()) {
      expect(scrollController.offset, greaterThan(100.0),
          reason:
              'Ballistic scroll expected on $debugDefaultTargetPlatformOverride');
    } else {
      expect(scrollController.offset, 100.0,
          reason:
              'Ballistic scroll not expected on $debugDefaultTargetPlatformOverride');
    }
    ...
    // Try flinging upward.
    await tester.flingFrom(
        const Offset(797.0, 545.0), const Offset(0, -60), 500.0);
    await tester.pumpAndSettle();
    if (isMobilePlatform()) {
      expect(scrollController.offset, lessThan(300.0),
          reason:
              'Ballistic scroll expected on $debugDefaultTargetPlatformOverride');
    } else {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.flingFrom(
          const Offset(45.0, 597.0), const Offset(80, 0.0), 500.0);
      await tester.pumpAndSettle();
      if (isMobilePlatform()) {
        expect(scrollController.offset, greaterThan(100.0),
            reason:
      ...
      await tester.flingFrom(
          const Offset(794.0, 597.0), const Offset(-80, 0), 500.0);
      await tester.pumpAndSettle();
      if (isMobilePlatform()) {
        expect(scrollController.offset, lessThan(100.0),
            reason:
                'Ballistic scroll expected on $debugDefaultTargetPlatformOverride');
      } else {
        expect(scrollController.offset, 100.0,
      ...
      // Try flinging to the right.
      await tester.flingFrom(
          const Offset(6.0, 597.0), const Offset(80, 0), 500.0);
      await tester.pumpAndSettle();
  ...
  testWidgets(
      'Safe to drag trackpad when maxScrollExtent is 0 (scrollbar is not painted)',
      (WidgetTester tester) async {
    ...
    await tester.trackpadFling(
        find.byType(SingleChildScrollView), const Offset(0, -100), 500);
    await tester.pumpAndSettle();
    expect(scrollController.offset, 0.0);
  ...
  testWidgets(
    ...
    await tester.trackpadFling(
        find.byType(SingleChildScrollView), const Offset(0, -600), 500);
    await tester.pumpAndSettle();
    ...
    await tester.trackpadFling(
        find.byType(SingleChildScrollView), const Offset(-800, 0), 500);
    await tester.pumpAndSettle();
    ...
    await tester.trackpadFling(
        find.byType(SingleChildScrollView), const Offset(-100, 0), 500);
    await tester.pumpAndSettle();
    expect(scrollController.offset, greaterThan(100));
  ...
  testWidgets(
    ...
    RawGestureDetector getScrollbarGestureDetector() {
      return tester.widget<RawGestureDetector>(find
          .descendant(
