...
class TestSemantics {
  ...
  bool _matches(
    ...
    if ((actionsBitmask & ~SemanticsAction.focus.index) !=
        (nodeData.actions & ~SemanticsAction.focus.index)) {
      return fail(
...
class SemanticsTester {
  ...
  Iterable<SemanticsNode> nodesWith({
    ...
    bool checkNode(SemanticsNode node) {
      ...
      if (actions != null) {
        ...
        // updated with the proper actions information for focus.
        // https://github.com/flutter/flutter/issues/149842
        final List<SemanticsAction> nonFocusActions = actions
            .where((SemanticsAction action) => action != SemanticsAction.focus)
            .toList();
        final int expectedActions = nonFocusActions.fold<int>(
            0, (int value, SemanticsAction action) => value | action.index);
        final int actualActions =
            node.getSemanticsData().actions & ~SemanticsAction.focus.index;
  ...
  static String _actionsToSemanticsActionExpression(dynamic actions) {
    ...
    // https://github.com/flutter/flutter/issues/149842
    if (actions is int) {
      list = SemanticsAction.values.where((SemanticsAction action) =>
          action != SemanticsAction.focus && (action.index & actions) != 0);
    } else {
...
class _IncludesNodeWith extends Matcher {
  ...
  String get _configAsString {
    final List<String> strings = <String>[
      ...
      // https://github.com/flutter/flutter/issues/149842
      if (actions != null)
        'actions "${actions!.where((SemanticsAction action) => action != SemanticsAction.focus).join(', ')}"',
