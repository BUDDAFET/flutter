...
class DelayedImageProvider extends ImageProvider<DelayedImageProvider> {
  ...
  ImageStreamCompleter loadImage(
      DelayedImageProvider key, ImageDecoderCallback decode) {
...
class MultiFrameImageProvider extends ImageProvider<MultiFrameImageProvider> {
  ...
  @override
  ImageStreamCompleter loadImage(
      MultiFrameImageProvider key, ImageDecoderCallback decode) {
    return completer;
  }
...
void main() {
  ...
  test('Decoration.lerp()', () {
    ...
    expect(
        c.color,
        equals(Color.lerp(
            const Color(0xFFFFFFFF), const Color(0x00000000), 0.25)));
  ...
  test('Decoration.lerp identical a,b', () {
    ...
    expect(identical(Decoration.lerp(decoration, decoration, 0.5), decoration),
        true);
  });
  ...
  test('BoxDecorationImageListener does not change when image is clone',
      () async {
    final ui.Image image1 =
        await createTestImage(width: 10, height: 10, cache: false);
    final ui.Image image2 =
        await createTestImage(width: 10, height: 10, cache: false);
    final MultiImageCompleter completer = MultiImageCompleter();
    final MultiFrameImageProvider imageProvider =
        MultiFrameImageProvider(completer);
    final DecorationImage backgroundImage =
        DecorationImage(image: imageProvider);
    ...
    void testDecoration(
      ...
      FakeAsync().run((FakeAsync async) async {
        ...
        final List<Invocation> commands =
          ...
          return invocation.memberName == #clipPath ||
              invocation.memberName == #drawImageRect;
        }).toList();
        if (expectClip) {
          // We expect a clip to precede the drawImageRect call.
  ...
  test('DecorationImage test', () async {
    const ColorFilter colorFilter =
        ui.ColorFilter.mode(Color(0xFF00FF00), BlendMode.src);
    final ui.Image image = await createTestImage(width: 100, height: 100);
    ...
    final TestCanvas canvas = TestCanvas();
    boxPainter.paint(canvas, Offset.zero,
        const ImageConfiguration(size: Size(100.0, 100.0)));

    final Invocation call = canvas.invocations
        .singleWhere((Invocation call) => call.memberName == #drawImageNine);
    expect(call.isMethod, isTrue);
    expect(call.positionalArguments, hasLength(4));
    expect(call.positionalArguments[0], isA<ui.Image>());
    expect(call.positionalArguments[1],
        const Rect.fromLTRB(10.0, 20.0, 40.0, 60.0));
    expect(call.positionalArguments[2],
  ...
  test(
    ...
    expect(
        error.diagnostics[3], isA<DiagnosticsProperty<ImageConfiguration>>());
    expect(
      error.toStringDeep(),
      'FlutterError\n'
  ...
  test('BoxDecoration.lerp - gradients', () {
    const Gradient gradient =
        LinearGradient(colors: <Color>[Color(0x00000000), Color(0xFFFFFFFF)]);
    expect(
      BoxDecoration.lerp(
        const BoxDecoration(),
      ...
      const BoxDecoration(
          gradient: LinearGradient(
              colors: <Color>[Color(0x00000000), Color(0x00FFFFFF)])),
    );
    expect(
      BoxDecoration.lerp(
        const BoxDecoration(),
    ...
    expect(
      ...
      const BoxDecoration(
          gradient: LinearGradient(
              colors: <Color>[Color(0x00000000), Color(0x40FFFFFF)])),
    );
    expect(
      BoxDecoration.lerp(
        const BoxDecoration(),
      ...
      const BoxDecoration(
          gradient: LinearGradient(
              colors: <Color>[Color(0x00000000), Color(0xBFFFFFFF)])),
    );
    expect(
      BoxDecoration.lerp(
        const BoxDecoration(),
  ...
  test('Decoration.lerp with unrelated decorations', () {
    expect(
        Decoration.lerp(
            const FlutterLogoDecoration(), const BoxDecoration(), 0.0),
        isA<FlutterLogoDecoration>());
    expect(
        Decoration.lerp(
            const FlutterLogoDecoration(), const BoxDecoration(), 0.25),
        isA<FlutterLogoDecoration>());
    expect(
        Decoration.lerp(
            const FlutterLogoDecoration(), const BoxDecoration(), 0.75),
        isA<BoxDecoration>());
    expect(
        Decoration.lerp(
            const FlutterLogoDecoration(), const BoxDecoration(), 1.0),
        isA<BoxDecoration>());
  });
  ...
  test('paintImage BoxFit.none scale test', () async {
    for (double scale = 1.0; scale <= 4.0; scale += 1.0) {
      ...
      final Invocation call = canvas.invocations
          .firstWhere((Invocation call) => call.memberName == #drawImageRect);
      ...
      final Rect expectedTileRect = Rect.fromPoints(
        outputRect.bottomRight
            .translate(-expectedTileSize.width, -expectedTileSize.height),
        outputRect.bottomRight,
      );
  ...
  test('DecorationImage scale test', () async {
    ...
    expect(
        call.positionalArguments[2], const Rect.fromLTRB(0.0, 0.0, 25.0, 25.0));
  });
  ...
  test('DecorationImagePainter disposes of image when disposed', () async {
    ...
    final int baselineRefCount =
        info.image.debugGetOpenHandleStackTraces()!.length;

    final DecorationImagePainter painter =
        DecorationImage(image: provider).createPainter(() {});
    final Canvas canvas = TestCanvas();
    painter.paint(canvas, Rect.zero, Path(), ImageConfiguration.empty);

    expect(info.image.debugGetOpenHandleStackTraces()!.length,
        baselineRefCount + 1);
    painter.dispose();
    expect(
        info.image.debugGetOpenHandleStackTraces()!.length, baselineRefCount);
  ...
  test('BoxShadow.copyWith', () {
    ...
    expect(const BoxShadow().copyWith(color: const Color(0xFF112233)),
        const BoxShadow(color: Color(0xFF112233)));
    expect(const BoxShadow(), isNot(const BoxShadow(offset: Offset(1.0, 2.0))));
    expect(const BoxShadow().copyWith(offset: const Offset(1.0, 2.0)),
    ...
    expect(
        const BoxShadow(), isNot(const BoxShadow(blurStyle: BlurStyle.outer)));
    expect(const BoxShadow().copyWith(blurStyle: BlurStyle.outer),
