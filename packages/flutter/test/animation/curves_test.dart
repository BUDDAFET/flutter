...
void main() {
  ...
  test('Invalid transform parameter should assert', () {
    ...
    expect(() => Curves.easeInOutCubicEmphasized.transform(-0.0001),
        throwsAssertionError);
    expect(() => Curves.easeInOutCubicEmphasized.transform(1.0001),
        throwsAssertionError);
  ...
  test('Split interpolates values properly', () {
    ...
    expect(
        curve.transform(0.5), moreOrLessEquals(0.760461, epsilon: tolerance));
    expect(
        curve.transform(0.75), moreOrLessEquals(0.962055, epsilon: tolerance));
  ...
  test('CatmullRomSpline interpolates values properly', () {
    ...
    expect(curve.transform(0.25).dx,
        moreOrLessEquals(0.0966945, epsilon: tolerance));
    expect(curve.transform(0.25).dy,
        moreOrLessEquals(0.2626806, epsilon: tolerance));
    ...
    expect(curve.transform(0.75).dx,
        moreOrLessEquals(0.570260, epsilon: tolerance));
    expect(curve.transform(0.75).dy,
        moreOrLessEquals(0.883085, epsilon: tolerance));
  ...
  test('CatmullRomCurve interpolates given points correctly', () {
    ...
    expect(curve.transform(0.01),
        moreOrLessEquals(0.012874734350170863, epsilon: tolerance));
    expect(curve.transform(0.2),
        moreOrLessEquals(0.24989646045277542, epsilon: tolerance));
    expect(curve.transform(0.33),
        moreOrLessEquals(0.250037698527661, epsilon: tolerance));
    expect(curve.transform(0.5),
        moreOrLessEquals(0.9999057323235939, epsilon: tolerance));
    expect(curve.transform(0.6),
        moreOrLessEquals(0.9357294964536621, epsilon: tolerance));
    expect(curve.transform(0.8),
        moreOrLessEquals(0.7500423402378034, epsilon: tolerance));
