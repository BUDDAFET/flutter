...
void main() {
  ...
  testWidgets('ExpansionTile initial state', (WidgetTester tester) async {
    ...
    expect(
        (expandedContainerDecoration.shape as Border).top.color, dividerColor);
    expect((expandedContainerDecoration.shape as Border).bottom.color,
        dividerColor);
    ...
    expect((collapsedContainerDecoration.shape as Border).top.color,
        Colors.transparent);
    expect((collapsedContainerDecoration.shape as Border).bottom.color,
        Colors.transparent);
    ...
    expect((collapsingContainerDecoration.shape as Border).top.color,
        const Color(0x15222222));
    expect((collapsingContainerDecoration.shape as Border).bottom.color,
        const Color(0x15222222));
    ...
    expect((expandedContainerDecoration.shape as Border).top.color,
        Colors.transparent);
    expect((expandedContainerDecoration.shape as Border).bottom.color,
        Colors.transparent);
    ...
    expect(collapsedContainerDecoration.color, Colors.transparent);
    expect(
        (collapsedContainerDecoration.shape as Border).top.color, dividerColor);
    expect((collapsedContainerDecoration.shape as Border).bottom.color,
        dividerColor);
  },
  ...
  testWidgets('ExpansionTile.collapsedBackgroundColor',
    ...
    ShapeDecoration shapeDecoration = tester
        .firstWidget<DecoratedBox>(find.descendant(
  ...
  testWidgets('ExpansionTile default iconColor, textColor',
      (WidgetTester tester) async {
  ...
  testWidgets('ExpansionTile Semantics announcement',
      (WidgetTester tester) async {
    final SemanticsHandle handle = tester.ensureSemantics();
    ...
    // "Expanded".
    expect(
        tester.takeAnnouncements().first.message, localizations.collapsedHint);
    ...
    // "Collapsed".
    expect(
        tester.takeAnnouncements().first.message, localizations.expandedHint);
    handle.dispose();
  },
      skip: defaultTargetPlatform ==
          TargetPlatform
              .iOS); // [intended] https://github.com/flutter/flutter/issues/122101.
  ...
  testWidgets('Semantics with the onTapHint is an ancestor of ListTile',
    ...
    expect(semantics.hintOverrides!.onTapHint,
  ...
  testWidgets('Collapsed ExpansionTile properties can be updated with setState',
    ...
    expect(material.clipBehavior, Clip.antiAlias);
    expect(tester.state<TestIconState>(find.byType(TestIcon)).iconTheme.color,
        const Color(0xffffffff));
    expect(tester.state<TestTextState>(find.byType(TestText)).textStyle.color,
        const Color(0xffffffff));
  ...
  testWidgets('Override ExpansionTile animation using AnimationStyle',
      (WidgetTester tester) async {
    const Key expansionTileKey = Key('expansionTileKey');
    ...
    await tester.pump(const Duration(
        milliseconds: 50)); // Advance the animation by 1/4 of its duration.

    expect(getHeight(expansionTileKey), closeTo(67.4, 0.1));

    await tester.pump(const Duration(
        milliseconds: 50)); // Advance the animation by 2/4 of its duration.
    ...
    await tester.pumpWidget(buildExpansionTile(
        animationStyle:
            AnimationStyle(duration: const Duration(milliseconds: 800))));
    await tester.pumpAndSettle();
    ...
    await tester.pump();
    await tester.pump(const Duration(
        milliseconds: 200)); // Advance the animation by 1/4 of its duration.

    expect(getHeight(expansionTileKey), closeTo(67.4, 0.1));

    await tester.pump(const Duration(
        milliseconds: 200)); // Advance the animation by 2/4 of its duration.
    ...
    // Test no animation.
    await tester.pumpWidget(
        buildExpansionTile(animationStyle: AnimationStyle.noAnimation));
  ...
  testWidgets(
      'Material3 - ExpansionTile draws Inkwell splash on top of background color',
      (WidgetTester tester) async {
    const Key expansionTileKey = Key('expansionTileKey');
    ...
    await expectLater(
      ...
      matchesGoldenFile(
          'expansion_tile.ink_splash.drawn_on_top_of_background_color.png'),
    );
  ...
  testWidgets('Default clipBehavior when a shape is provided',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
  ...
  testWidgets('Can override clipBehavior when a shape is provided',
      (WidgetTester tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
  ...
  group('Material 2', () {
    ...
    testWidgets(
        'Material2 - ExpansionTile draws inkwell splash on top of background color',
        (WidgetTester tester) async {
      const Key expansionTileKey = Key('expansionTileKey');
      final ThemeData theme = ThemeData(useMaterial3: false);
      const ShapeBorder shape = RoundedRectangleBorder(
      ...
      await tester.pump(); // Start the splash animation.
      await tester
          .pump(const Duration(milliseconds: 100)); // Splash is underway.
  ...
  testWidgets('Call to ExpansionTileController.of()',
      (WidgetTester tester) async {
    ...
    final ExpansionTileController controller1 =
        ExpansionTileController.of(childKey.currentContext!);
    expect(controller1.isExpanded, isTrue);

    final ExpansionTileController controller2 =
        ExpansionTileController.of(titleKey.currentContext!);
  ...
  testWidgets('Call to ExpansionTile.maybeOf()', (WidgetTester tester) async {
    ...
    final ExpansionTileController? controller1 =
        ExpansionTileController.maybeOf(titleKey.currentContext!);
    expect(controller1, isNotNull);
    expect(controller1?.isExpanded, isFalse);

    final ExpansionTileController? controller2 =
        ExpansionTileController.maybeOf(nonDescendantKey.currentContext!);
    expect(controller2, isNull);
  });

  testWidgets(
      'Check if dense, enableFeedback, visualDensity parameter is working',
      (WidgetTester tester) async {
    final GlobalKey titleKey = GlobalKey();
    final GlobalKey nonDescendantKey = GlobalKey();
  ...
  testWidgets('ExpansionTileController should not toggle if disabled',
      (WidgetTester tester) async {
    final ExpansionTileController controller = ExpansionTileController();
