...
void main() {
  test('SliderThemeData copyWith, ==, hashCode basics', () {
    ...
    expect(const SliderThemeData().hashCode,
        const SliderThemeData().copyWith().hashCode);
  });
  ...
  testWidgets('Default SliderThemeData debugFillProperties',
  ...
  testWidgets('SliderThemeData implements debugFillProperties',
      (WidgetTester tester) async {
    final DiagnosticPropertiesBuilder builder = DiagnosticPropertiesBuilder();
  ...
  testWidgets('Slider defaults', (WidgetTester tester) async {
    ...
    try {
      ...
      expect(
        ...
        paints
          ..rrect(
              rrect: RRect.fromLTRBAndCorners(24.0, 297.0, 362.4, 303.0,
                  topLeft: activatedRadius, bottomLeft: activatedRadius),
              color: activeTrackColor)
          ..rrect(
              rrect: RRect.fromLTRBAndCorners(362.4, 298.0, 776.0, 302.0,
      ...
      expect(material,
          isNot(paints..rrect(color: disabledSecondaryActiveTrackColor)));
  ...
  testWidgets('Slider uses the right theme colors for the right components',
    ...
    try {
      ...
      expect(material,
          isNot(paints..rrect(color: sliderTheme.disabledActiveTrackColor)));
      expect(material,
          isNot(paints..rrect(color: sliderTheme.disabledInactiveTrackColor)));
      expect(
          material,
          isNot(paints
      ...
      expect(material, isNot(paints..circle(color: sliderTheme.thumbColor)));
      expect(
          material, isNot(paints..rrect(color: sliderTheme.activeTrackColor)));
      expect(material,
          isNot(paints..rrect(color: sliderTheme.inactiveTrackColor)));
      expect(material,
          isNot(paints..rrect(color: sliderTheme.secondaryActiveTrackColor)));
  ...
  testWidgets('Slider parameters overrides theme properties',
      (WidgetTester tester) async {
    debugDisableShadows = false;
  ...
  testWidgets('Slider uses ThemeData slider theme if present',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
    ...
    await tester.pumpWidget(_buildApp(sliderTheme,
        value: 0.5, secondaryTrackValue: 0.75, enabled: false));
    final MaterialInkController material =
        Material.of(tester.element(find.byType(Slider)));
  ...
  testWidgets(
    ...
    final SliderThemeData sliderTheme = SliderThemeData.fromPrimaryColors(
      ...
      valueIndicatorTextStyle: ThemeData.fallback()
          .textTheme
          .bodyLarge!
          .copyWith(color: customColor4),
    );

    expect(sliderTheme.activeTrackColor, equals(customColor1.withAlpha(0xff)));
    expect(
        sliderTheme.inactiveTrackColor, equals(customColor1.withAlpha(0x3d)));
    expect(sliderTheme.secondaryActiveTrackColor,
        equals(customColor1.withAlpha(0x8a)));
    expect(sliderTheme.disabledActiveTrackColor,
        equals(customColor2.withAlpha(0x52)));
    expect(sliderTheme.disabledInactiveTrackColor,
        equals(customColor2.withAlpha(0x1f)));
    expect(sliderTheme.disabledSecondaryActiveTrackColor,
        equals(customColor2.withAlpha(0x1f)));
    expect(
        sliderTheme.activeTickMarkColor, equals(customColor3.withAlpha(0x8a)));
    expect(sliderTheme.inactiveTickMarkColor,
        equals(customColor1.withAlpha(0x8a)));
    expect(sliderTheme.disabledActiveTickMarkColor,
        equals(customColor3.withAlpha(0x1f)));
    expect(sliderTheme.disabledInactiveTickMarkColor,
        equals(customColor2.withAlpha(0x1f)));
    expect(sliderTheme.thumbColor, equals(customColor1.withAlpha(0xff)));
    expect(
        sliderTheme.disabledThumbColor, equals(customColor2.withAlpha(0x52)));
    expect(sliderTheme.overlayColor, equals(customColor1.withAlpha(0x1f)));
    expect(
        sliderTheme.valueIndicatorColor, equals(customColor1.withAlpha(0xff)));
    expect(sliderTheme.valueIndicatorStrokeColor,
        equals(customColor1.withAlpha(0xff)));
  ...
  testWidgets('SliderThemeData generates correct shapes for fromPrimaryColors',
    ...
    expect(
        sliderTheme.rangeTickMarkShape, const RoundRangeSliderTickMarkShape());
    expect(sliderTheme.rangeThumbShape, const RoundRangeSliderThumbShape());
    expect(
        sliderTheme.rangeTrackShape, const RoundedRectRangeSliderTrackShape());
    expect(sliderTheme.rangeValueIndicatorShape,
  ...
  testWidgets('SliderThemeData lerps correctly', (WidgetTester tester) async {
    ...
    final SliderThemeData lerp =
        SliderThemeData.lerp(sliderThemeBlack, sliderThemeWhite, 0.5);
    ...
    expect(lerp.disabledSecondaryActiveTrackColor,
        equals(middleGrey.withAlpha(0x1f)));
    ...
    expect(
        lerp.disabledActiveTickMarkColor, equals(middleGrey.withAlpha(0x1f)));
    expect(
        lerp.disabledInactiveTickMarkColor, equals(middleGrey.withAlpha(0x1f)));
    ...
    expect(lerp.valueIndicatorTextStyle!.color,
        equals(middleGrey.withAlpha(0xff)));
  });

  testWidgets('Default slider track draws correctly',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
      primarySwatch: Colors.blue,
    ...
    final SliderThemeData sliderTheme =
        theme.sliderTheme.copyWith(thumbColor: Colors.red.shade500);

    await tester.pumpWidget(
        _buildApp(sliderTheme, value: 0.25, secondaryTrackValue: 0.5));
    final MaterialInkController material =
        Material.of(tester.element(find.byType(Slider)));
    ...
    expect(
      ...
      paints
        ..rrect(
            rrect: RRect.fromLTRBAndCorners(24.0, 297.0, 212.0, 303.0,
                topLeft: activatedRadius, bottomLeft: activatedRadius),
            color: sliderTheme.activeTrackColor)
        ..rrect(
            rrect: RRect.fromLTRBAndCorners(212.0, 298.0, 776.0, 302.0,
                topRight: radius, bottomRight: radius),
    ...
    await tester.pumpWidget(_buildApp(sliderTheme,
        value: 0.25, secondaryTrackValue: 0.5, enabled: false));
  ...
  testWidgets('Default slider overlay draws correctly',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
      primarySwatch: Colors.blue,
  ...
  testWidgets('Slider can use theme overlay with material states',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
      primarySwatch: Colors.blue,
  ...
  testWidgets('Default slider ticker and thumb shape draw correctly',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
      primarySwatch: Colors.blue,
    ...
    expect(
        material, paints..circle(color: sliderTheme.thumbColor, radius: 10.0));

    await tester
        .pumpWidget(_buildApp(sliderTheme, value: 0.45, enabled: false));
    await tester.pumpAndSettle(); // wait for disable animation

    expect(material,
        paints..circle(color: sliderTheme.disabledThumbColor, radius: 10.0));
    ...
    await tester.pumpWidget(
        _buildApp(sliderTheme, value: 0.45, divisions: 3, enabled: false));
    await tester.pumpAndSettle(); // wait for disable animation
  ...
  testWidgets('Default paddle slider value indicator shape draws correctly',
    ...
    try {
      ...
      expect(
        ...
        paints
          ..path(
            ...
            excludes: <Offset>[
              const Offset(98.1, -40.0),
              const Offset(-20.1, -40.0)
            ],
          ),
      );
      await gesture.up();
      ...
      await tester.pumpWidget(buildApp('1000000',
          sliderValue: 0.0, textScaler: const TextScaler.linear(0.5)));
      center = tester.getCenter(find.byType(Slider));
      gesture = await tester.startGesture(center);
      // Wait for value indicator animation to finish.
      await tester.pumpAndSettle();
      ...
      await tester.pumpWidget(buildApp('1000000',
          sliderValue: 0.0, textScaler: const TextScaler.linear(2.5)));
      center = tester.getCenter(find.byType(Slider));
      gesture = await tester.startGesture(center);
      // Wait for value indicator animation to finish.
      await tester.pumpAndSettle();
  ...
  testWidgets('The slider track height can be overridden',
      (WidgetTester tester) async {
    final SliderThemeData sliderTheme =
        ThemeData().sliderTheme.copyWith(trackHeight: 16);
    ...
    expect(
      ...
      paints
        ..rrect(
            rrect: RRect.fromLTRBAndCorners(24.0, 291.0, 212.0, 309.0,
                topLeft: activatedRadius, bottomLeft: activatedRadius),
            color: sliderTheme.activeTrackColor)
        ..rrect(
            rrect: RRect.fromLTRBAndCorners(212.0, 292.0, 776.0, 308.0,
                topRight: radius, bottomRight: radius),
            color: sliderTheme.inactiveTrackColor),
    );

    await tester
        .pumpWidget(_buildApp(sliderTheme, value: 0.25, enabled: false));
    await tester.pumpAndSettle(); // wait for disable animation
  ...
  testWidgets('The default slider thumb shape sizes can be overridden',
    ...
    expect(
      ...
      paints
        ..circle(
            x: 212, y: 300, radius: 11, color: sliderTheme.disabledThumbColor),
    );
  });
  ...
  testWidgets(
    ...
    expect(
      ...
      paints
        ..circle(
            x: 212, y: 300, radius: 9, color: sliderTheme.disabledThumbColor),
    );
  });
  ...
  testWidgets('The default slider tick mark shape size can be overridden',
    ...
    expect(
      ...
      paints
        ..circle(
            x: 26, y: 300, radius: 5, color: sliderTheme.activeTickMarkColor)
        ..circle(
            x: 400, y: 300, radius: 5, color: sliderTheme.activeTickMarkColor)
        ..circle(
            x: 774,
            y: 300,
            radius: 5,
    ...
    await tester.pumpWidget(
        _buildApp(sliderTheme, value: 0.5, divisions: 2, enabled: false));
    await tester.pumpAndSettle();
    ...
    expect(
      ...
      paints
        ..circle(
            ...
            y: 300,
            radius: 5,
            color: sliderTheme.disabledActiveTickMarkColor)
        ..circle(
            x: 400,
            y: 300,
            radius: 5,
        ...
        ..circle(
            ...
            y: 300,
            radius: 5,
            color: sliderTheme.disabledInactiveTickMarkColor),
    );
  });
  ...
  testWidgets(
    ...
    await tester.pumpWidget(
        _buildRangeApp(sliderTheme, values: const RangeValues(0.0, 1.0)));
    ...
    expect(
      ...
      paints
        ...
        ..rect(
          rect: const Rect.fromLTRB(10.0, 297.0, 790.0, 303.0),
        )
        // inactive track RRect. Ends 10 pixels from right of screen.
        ..rrect(
            rrect: RRect.fromLTRBAndCorners(
  ...
  // [ShowValueIndicator].
  testWidgets('The slider can skip all of its component painting',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'The slider can skip all component painting except the tick marks',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'The slider can skip all component painting except the value indicator',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'PaddleSliderValueIndicatorShape skips all painting at zero scale',
      (WidgetTester tester) async {
    // Pump a slider with just a value indicator.
    await tester.pumpWidget(_buildApp(
      ThemeData().sliderTheme.copyWith(
  ...
  testWidgets(
      'Default slider value indicator shape skips all painting at zero scale',
      (WidgetTester tester) async {
    // Pump a slider with just a value indicator.
    await tester.pumpWidget(_buildApp(
      ThemeData().sliderTheme.copyWith(
  ...
  testWidgets(
      'Default paddle range slider value indicator shape draws correctly',
      (WidgetTester tester) async {
    debugDisableShadows = false;
    try {
      final ThemeData theme = ThemeData(
  ...
  testWidgets('SliderTheme.allowedInteraction is themeable',
      (WidgetTester tester) async {
    double value = 0.0;
  ...
  testWidgets(
      'PaddleSliderValueIndicatorShape supports SliderTheme.valueIndicatorStrokeColor',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      sliderTheme: const SliderThemeData(
        showValueIndicator: ShowValueIndicator.always,
  ...
  testWidgets(
      'DropSliderValueIndicatorShape supports SliderTheme.valueIndicatorStrokeColor',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      sliderTheme: const SliderThemeData(
        showValueIndicator: ShowValueIndicator.always,
  ...
  group('Material 2', () {
    ...
    testWidgets('Slider defaults', (WidgetTester tester) async {
      ...
      try {
        ...
        expect(material, isNot(paints..rrect(color: disabledActiveTrackColor)));
        expect(
            material, isNot(paints..rrect(color: disabledInactiveTrackColor)));
        expect(material,
            isNot(paints..rrect(color: disabledSecondaryActiveTrackColor)));
        expect(material, isNot(paints..circle(color: activeTickMarkColor)));
        expect(material, isNot(paints..circle(color: inactiveTickMarkColor)));
        ...
        expect(material, isNot(paints..rrect(color: inactiveTrackColor)));
        expect(
            material, isNot(paints..rrect(color: secondaryActiveTrackColor)));
...
Widget _buildRangeApp(
  ...
  final ValueChanged<RangeValues>? onChanged =
      enabled ? (RangeValues d) => values = d : null;
  return MaterialApp(
    home: Scaffold(
      body: Center(
        child: SliderTheme(
