...
void main() {
  test('ChipThemeData copyWith, ==, hashCode basics', () {
    ...
    expect(const ChipThemeData().hashCode,
        const ChipThemeData().copyWith().hashCode);
  });
  ...
  testWidgets('Default ChipThemeData debugFillProperties',
  ...
  testWidgets('ChipThemeData implements debugFillProperties',
      (WidgetTester tester) async {
    final DiagnosticPropertiesBuilder builder = DiagnosticPropertiesBuilder();
    const ChipThemeData(
  ...
  testWidgets('Material3 - Chip uses ThemeData chip theme',
      (WidgetTester tester) async {
    ...
    expect(tester.getSize(find.byType(RawChip)),
        const Size(402, 252)); // label + padding + labelPadding
    expect(
      getMaterial(tester).shape,
      chipTheme.shape
          ?.copyWith(side: BorderSide(color: theme.colorScheme.outline)),
    );
    expect(getLabelStyle(tester).style.fontSize, 32);
    expect(getIconData(tester).color, chipTheme.iconTheme!.color);
  });

  testWidgets('Material2 - Chip uses ThemeData chip theme',
      (WidgetTester tester) async {
    const ChipThemeData chipTheme = ChipThemeData(
      backgroundColor: Color(0xff112233),
      elevation: 4,
    ...
    expect(getMaterial(tester).elevation, chipTheme.elevation);
    expect(tester.getSize(find.byType(RawChip)),
        const Size(400, 250)); // label + padding + labelPadding
    expect(getMaterial(tester).shape, chipTheme.shape);
    expect(getLabelStyle(tester).style.fontSize, 32);
    expect(getIconData(tester).color, chipTheme.iconTheme!.color);
  });

  testWidgets('Material3 - Chip uses local ChipTheme',
      (WidgetTester tester) async {
    const ChipThemeData chipTheme = ChipThemeData(
      backgroundColor: Color(0xff112233),
      elevation: 4,
  ...
  testWidgets('Material2 - Chip uses local ChipTheme',
      (WidgetTester tester) async {
    const ChipThemeData chipTheme = ChipThemeData(
      backgroundColor: Color(0xff112233),
      elevation: 4,
  ...
  testWidgets('Chip properties overrides ChipTheme',
      (WidgetTester tester) async {
    const ChipThemeData chipTheme = ChipThemeData(
      backgroundColor: Color(0xff112233),
      elevation: 4,
    ...
    const RoundedRectangleBorder shape =
        RoundedRectangleBorder(side: BorderSide(color: Color(0xff0000ff)));
    ...
    expect(tester.getSize(find.byType(RawChip)),
        const Size(242, 132)); // label + padding + labelPadding
  ...
  testWidgets('Material3 - Chip uses constructor parameters',
      (WidgetTester tester) async {
    const Color backgroundColor = Color(0xff332211);
    const double elevation = 3;
    const double fontSize = 32;
    const OutlinedBorder shape =
        CircleBorder(side: BorderSide(color: Color(0xff0000ff)));
  ...
  testWidgets('Material2 - Chip uses constructor parameters',
      (WidgetTester tester) async {
    const Color backgroundColor = Color(0xff332211);
    const double elevation = 3;
    const double fontSize = 32;
  ...
  testWidgets('ChipTheme.fromDefaults', (WidgetTester tester) async {
    ...
    expect(chipTheme.labelStyle,
        labelStyle.copyWith(color: Colors.black.withAlpha(0xde)));
    expect(chipTheme.secondaryLabelStyle,
        labelStyle.copyWith(color: Colors.red.withAlpha(0xde)));
    ...
    expect(chipTheme.secondarySelectedColor,
        Colors.tealAccent[200]!.withAlpha(0x3d));
    expect(chipTheme.shadowColor, Colors.black);
    expect(chipTheme.selectedShadowColor, Colors.black);
    expect(chipTheme.showCheckmark, true);
    expect(chipTheme.checkmarkColor, null);
    ...
    expect(chipTheme.shape, null);
    expect(chipTheme.labelStyle,
        labelStyle.copyWith(color: Colors.white.withAlpha(0xde)));
    expect(chipTheme.secondaryLabelStyle,
  ...
  testWidgets('ChipThemeData generates correct opacities for defaults',
    ...
    final TextStyle customStyle =
        ThemeData.fallback().textTheme.bodyLarge!.copyWith(color: customColor2);
    ...
    expect(lightTheme.secondarySelectedColor,
        equals(customColor1.withAlpha(0x3d)));
    ...
    expect(lightTheme.secondaryLabelStyle?.color,
        equals(customColor1.withAlpha(0xde)));
    ...
    expect(
        darkTheme.secondarySelectedColor, equals(customColor1.withAlpha(0x3d)));
    ...
    expect(darkTheme.secondaryLabelStyle?.color,
        equals(customColor1.withAlpha(0xde)));
    ...
    expect(customTheme.secondarySelectedColor,
        equals(customColor2.withAlpha(0x3d)));
    ...
    expect(customTheme.secondaryLabelStyle?.color,
        equals(customColor2.withAlpha(0xde)));
  ...
  testWidgets('ChipThemeData lerps correctly', (WidgetTester tester) async {
    ...
    final ChipThemeData lerp =
        ChipThemeData.lerp(chipThemeBlack, chipThemeWhite, 0.5);
    ...
    final ChipThemeData lerpANull25 =
        ChipThemeData.lerp(null, chipThemeWhite, 0.25);
    ...
    expect(lerpANull25.secondarySelectedColor,
        equals(Colors.white.withAlpha(0x0f)));
    ...
    expect(
        lerpANull25.selectedShadowColor, equals(Colors.white.withAlpha(0x40)));
    expect(lerpANull25.showCheckmark, equals(true));
    expect(lerpANull25.labelPadding,
        equals(const EdgeInsets.only(top: 2.0, bottom: 2.0)));
    ...
    expect(lerpANull25.secondaryLabelStyle?.color,
        equals(Colors.white.withAlpha(0x38)));
    ...
    final ChipThemeData lerpANull75 =
        ChipThemeData.lerp(null, chipThemeWhite, 0.75);
    ...
    expect(lerpANull75.secondarySelectedColor,
        equals(Colors.white.withAlpha(0x2e)));
    ...
    expect(
        lerpANull75.selectedShadowColor, equals(Colors.white.withAlpha(0xbf)));
    expect(lerpANull75.showCheckmark, equals(true));
    expect(lerpANull75.labelPadding,
        equals(const EdgeInsets.only(top: 6.0, bottom: 6.0)));
    ...
    expect(lerpANull75.secondaryLabelStyle?.color,
        equals(Colors.white.withAlpha(0xa7)));
    ...
    final ChipThemeData lerpBNull25 =
        ChipThemeData.lerp(chipThemeBlack, null, 0.25);
    ...
    expect(lerpBNull25.secondarySelectedColor,
        equals(Colors.black.withAlpha(0x2e)));
    ...
    expect(
        lerpBNull25.selectedShadowColor, equals(Colors.black.withAlpha(0xbf)));
    expect(lerpBNull25.showCheckmark, equals(false));
    expect(lerpBNull25.labelPadding,
        equals(const EdgeInsets.only(left: 6.0, right: 6.0)));
    ...
    expect(lerpBNull25.secondaryLabelStyle?.color,
        equals(Colors.black.withAlpha(0xa7)));
    ...
    final ChipThemeData lerpBNull75 =
        ChipThemeData.lerp(chipThemeBlack, null, 0.75);
    ...
    expect(lerpBNull75.secondarySelectedColor,
        equals(Colors.black.withAlpha(0x0f)));
    ...
    expect(
        lerpBNull75.selectedShadowColor, equals(Colors.black.withAlpha(0x40)));
    expect(lerpBNull75.showCheckmark, equals(true));
    expect(lerpBNull75.labelPadding,
        equals(const EdgeInsets.only(left: 2.0, right: 2.0)));
    ...
    expect(lerpBNull75.secondaryLabelStyle?.color,
        equals(Colors.black.withAlpha(0x38)));
  ...
  testWidgets('Chip uses stateful color from chip theme',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'Material2 - Chip uses stateful border side from resolveWith pattern',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'Material3 - Chip uses stateful border side from resolveWith pattern',
      (WidgetTester tester) async {
    const Color selectedColor = Color(0x00000001);
    const Color defaultColor = Color(0x00000002);
  ...
  testWidgets('Material2 - Chip uses stateful shape from chip theme',
      (WidgetTester tester) async {
  ...
  testWidgets('Material3 - Chip uses stateful shape from chip theme',
      (WidgetTester tester) async {
    OutlinedBorder? getShape(Set<MaterialState> states) {
      if (states.contains(MaterialState.selected)) {
  ...
  testWidgets('RawChip uses material state color from ChipTheme',
      (WidgetTester tester) async {
  ...
  testWidgets('RawChip uses state colors from ChipTheme',
      (WidgetTester tester) async {
    const ChipThemeData chipTheme = ChipThemeData(
    ...
    expect(getMaterialBox(tester),
        paints..rrect(color: chipTheme.backgroundColor));
    ...
    // Disabled chip should have the provided disabledColor.
    expect(
        getMaterialBox(tester), paints..rrect(color: chipTheme.disabledColor));
    ...
    // Enabled & selected chip should have the provided selectedColor.
    expect(
        getMaterialBox(tester), paints..rrect(color: chipTheme.selectedColor));
  });

  // This is a regression test for https://github.com/flutter/flutter/issues/119163.
  testWidgets('RawChip respects checkmark properties from ChipTheme',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'Material3 - ChipThemeData.iconTheme respects default iconTheme.size',
      (WidgetTester tester) async {
  ...
  testWidgets(
      'ChipThemeData.avatarBoxConstraints updates avatar size constraints',
      (WidgetTester tester) async {
    ...
    final Offset chipTopLeft =
        tester.getTopLeft(find.byWidget(getMaterial(tester)));
  ...
  testWidgets(
      'ChipThemeData.deleteIconBoxConstraints updates delete icon size constraints',
      (WidgetTester tester) async {
    const double border = 1.0;
    const double iconSize = 18.0;
    const double labelPadding = 8.0;
  ...
  testWidgets(
      'ChipThemeData.deleteIconColor overrides ChipThemeData.iconTheme color',
      (WidgetTester tester) async {
