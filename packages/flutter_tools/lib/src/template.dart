...
class TemplatePathProvider {
  ...
  Future<Directory> imageDirectory(
    ...
    return name == null
        ? templateDirectory
        : templateDirectory.childDirectory(name);
  }
}
...
class Template {
  ...
  static Future<Template> merged(
    ...
    return Template._(
      ...
      <Directory>[
        for (final String name in names)
          if ((await templatePathProvider.imageDirectory(
                  name, fileSystem, logger))
              .existsSync())
            await templatePathProvider.imageDirectory(name, fileSystem, logger),
      ],
      fileSystem: fileSystem,
    ...
    _templateFilePaths
        ...
        final List<File> potentials = <File>[
          for (final Directory imageSourceDir in imageSourceDirectories)
            _fileSystem.file(_fileSystem.path.join(
                imageSourceDir.path,
                relativeDestinationPath.replaceAll(
                    imageTemplateExtension, ''))),
        ];
      ...
      if (sourceFile.path.endsWith(templateExtension)) {
        ...
        final String renderedContents =
            _templateRenderer.renderString(templateContents, localContext);
