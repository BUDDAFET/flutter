...
class IOSSimulatorUtils {
  ...
  Future<List<IOSSimulator>> getAttachedDevices() async {
    ...
    return connected
        .map<IOSSimulator?>((BootedSimDevice device) {
...
class SimControl {
    ...
    return <BootedSimDevice>[
      for (final String deviceCategory in devicesSection.keys)
        if (devicesSection[deviceCategory] case final List<Object?> devicesData)
          for (final Object? data
              in devicesData.map<Map<String, Object?>?>(castStringKeyedMap))
            if (data is Map<String, Object?>)
              BootedSimDevice(deviceCategory, data),
    ];
  }
  ...
  Future<RunResult> install(String deviceId, String appPath) async {
    ...
    try {
      ...
      throwToolExit(
          'Unable to install $appPath on $deviceId. This is sometimes caused by a malformed plist file:\n$exception');
    }
    return result;
  }
  ...
  Future<RunResult> stopApp(String deviceId, String appIdentifier) async {
    ...
    try {
      ...
      throwToolExit(
          'Unable to terminate $appIdentifier on $deviceId:\n$exception');
    }
    return result;
  }
  ...
  Future<List<IOSSimulatorRuntime>> listAvailableIOSRuntimes() async {
    ...
    try {
      final Object? decodeResult =
          (json.decode(results.stdout) as Map<String, Object?>)['runtimes'];
...
class IOSSimulator extends Device {
  ...
  Future<LaunchResult> startApp(
    ...
    if (!prebuiltApplication && package is BuildableIOSApp) {
      ...
      try {
        await _setupUpdatedApplicationBundle(
            package, debuggingOptions.buildInfo, mainPath);
      } on ToolExit catch (e) {
    ...
    try {
      ...
      final String plistPath =
          globals.fs.path.join(package.simulatorBundlePath, 'Info.plist');
      final String? bundleIdentifier = globals.plistParser
  ...
  Future<void> _setupUpdatedApplicationBundle(
      BuildableIOSApp app, BuildInfo buildInfo, String? mainPath) async {
  ...
  String get logFilePath {
    final String? logPath =
        globals.platform.environment['IOS_SIMULATOR_LOG_FILE_PATH'];
    return logPath != null
  ...
  DeviceLogReader getLogReader({
    ...
    assert(
        !includePastLogs, 'Past log reading not supported on iOS simulators.');
    return _logReaders.putIfAbsent(
        app, () => _IOSSimulatorLogReader(this, app));
  }
...
class IOSSimulatorRuntime {
  ...
  factory IOSSimulatorRuntime.fromJson(Map<String, Object?> data) {
    return IOSSimulatorRuntime._(
      ...
      isInternal:
          data['isInternal'] is bool? ? data['isInternal'] as bool? : null,
      isAvailable:
          data['isAvailable'] is bool? ? data['isAvailable'] as bool? : null,
...
Future<Process?> launchSystemLogTool(IOSSimulator device) async {
  ...
  if (await device.sdkMajorVersion < 11) {
    return globals.processUtils.start(
...
class _IOSSimulatorLogReader extends DeviceLogReader {
  ...
  static final RegExp _mapRegex =
      RegExp(r'\S+ +\S+ +(?:\S+) (.+?(?=\[))\[\d+\]\)?: (\(.*?\))? *(.*)$');

  // Jan 31 19:23:28 --- last message repeated 1 time ---
  static final RegExp _lastMessageSingleRegex =
      RegExp(r'\S+ +\S+ +\S+ --- last message repeated 1 time ---$');
  static final RegExp _lastMessageMultipleRegex =
      RegExp(r'\S+ +\S+ +\S+ --- last message repeated (\d+) times ---$');
  ...
  String? _filterDeviceLine(String string) {
    ...
    if (match != null) {
      ...
      // Filter out some messages that clearly aren't related to Flutter.
      if (string
          .contains(': could not find icon for representation -> com.apple.')) {
        return null;
      }
      ...
      if (appName == null) {
        return '$category: $content';
          ...
          (category == appName || category.endsWith(' $appName'))) {
        return content;
      }
    ...
    if (RegExp(r'assertion failed: .* libxpc.dylib .* 0x7d$')
  ...
  static final RegExp _unifiedLoggingEventMessageRegex =
      RegExp(r'.*"eventMessage" : (".*")');
