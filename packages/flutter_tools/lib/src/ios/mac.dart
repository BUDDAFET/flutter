...
const String kConcurrentRunFailureMessage2 =
  ...
  IMobileDevice({
    ...
    required Logger logger,
        ...
        _idevicescreenshotPath =
            artifacts.getHostArtifact(HostArtifact.idevicescreenshot).path,
...
Future<XcodeBuildResult> buildXcodeProject({
  ...
  final List<ProjectMigrator> migrators = <ProjectMigrator>[
    RemoveFrameworkLinkAndEmbeddingMigration(
        app.project, globals.logger, globals.flutterUsage, globals.analytics),
    ...
    if (project.usesSwiftPackageManager &&
        app.project.flutterPluginSwiftPackageManifest.existsSync())
  ...
  await removeFinderExtendedAttributes(
      app.project.parent.directory, globals.processUtils, globals.logger);
  ...
  final String? buildName =
      parsedBuildName(manifest: manifest, buildInfo: buildInfo);
  ...
  final String? buildNumber =
      parsedBuildNumber(manifest: manifest, buildInfo: buildInfo);
  ...
  if (autoSigningConfigs != null) {
    for (final MapEntry<String, String> signingConfig
        in autoSigningConfigs.entries) {
  ...
  if (workspacePath != null) {
    buildCommands.addAll(<String>[
      ...
      if (buildAction !=
          XcodeBuildAction
              .archive) // dSYM files aren't copied to the archive if BUILD_DIR is set.
  ...
  try {
    if (globals.logger.hasTerminal) {
      ...
      buildCommands.add(
          'SCRIPT_OUTPUT_STREAM_FILE=${scriptOutputPipeFile.absolute.path}');
    }

    final Directory resultBundleDirectory =
    ...
    buildCommands
        .addAll(environmentVariablesAsXcodeBuildSettings(globals.platform));
    ...
    buildResult =
        await _runBuildWithRetries(buildCommands, app, resultBundleDirectory);
    ...
    initialBuildStatus = null;
    globals.printStatus(
      'Xcode ${xcodeBuildActionToString(buildAction)} done.'
              .padRight(kDefaultStatusPadding + 1) +
          getElapsedAsSeconds(sw.elapsed).padLeft(5),
    );
    final Duration elapsedDuration = sw.elapsed;
    globals.flutterUsage.sendTiming(
        xcodeBuildActionToString(buildAction), 'xcode-ios', elapsedDuration);
    ...
    if (tempDir.existsSync()) {
      ...
      if (!resultBundle.existsSync()) {
        ...
        final XCResultIssueDiscarder warningDiscarder =
            XCResultIssueDiscarder(typeMatcher: XCResultIssueType.warning);
        final XCResultIssueDiscarder dartBuildErrorDiscarder =
  ...
  if (buildResult != null && buildResult.exitCode != 0) {
    ...
    if (buildAction == XcodeBuildAction.build) {
      ...
      if (hasWatchCompanion && environmentType == EnvironmentType.simulator) {
        ...
        targetBuildDir =
            targetBuildDir.replaceFirst('iphoneos', 'iphonesimulator');
      }
...
Future<void> removeFinderExtendedAttributes(FileSystemEntity projectDirectory,
    ProcessUtils processUtils, Logger logger) async {
  ...
  if (!success) {
    logger.printTrace(
        'Failed to remove xattr com.apple.FinderInfo from ${projectDirectory.path}');
  }
}

Future<RunResult?> _runBuildWithRetries(List<String> buildCommands,
    BuildableIOSApp app, Directory resultBundleDirectory) async {
...
_XCResultIssueHandlingResult _handleXCResultIssue({
  ...
  if (message == null) {
    return _XCResultIssueHandlingResult(
        requiresProvisioningProfile: false, hasProvisioningProfileIssue: false);
  }
  ...
  if (message.toLowerCase().contains('requires a provisioning profile.')) {
    return _XCResultIssueHandlingResult(
        requiresProvisioningProfile: true, hasProvisioningProfileIssue: true);
  } else if (message.toLowerCase().contains('provisioning profile')) {
    return _XCResultIssueHandlingResult(
        requiresProvisioningProfile: false, hasProvisioningProfileIssue: true);
...
Future<bool> _handleIssues(
  ...
  if (xcResult != null && xcResult.parseSuccess) {
    ...
    globals
        .printTrace('XCResult parsing error: ${xcResult.parsingErrorMessage}');
  }
  ...
  if (requiresProvisioningProfile) {
    ...
    logger.printError(missingPlatformInstructions(missingPlatform),
        emphasis: true);
...
Future<bool> _isPluginSwiftPackageOnly({
  ...
  final bool podspecExists =
      podspecPath != null && fileSystem.file(podspecPath).existsSync();
...
void _parseIssueInStdout(XcodeBuildExecution xcodeBuildExecution, Logger logger,
    XcodeBuildResult result) {
