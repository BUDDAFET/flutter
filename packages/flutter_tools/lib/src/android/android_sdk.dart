...
class AndroidSdk {
  ...
  static bool sdkDirectoryHasPlatformTools(String dir) {
    return globals.fs
        .isDirectorySync(globals.fs.path.join(dir, 'platform-tools'));
  }
  ...
  late final String? adbPath =
      getPlatformToolsPath(globals.platform.isWindows ? 'adb.exe' : 'adb');
  ...
  String? getPlatformToolsPath(String binaryName) {
    final File cmdlineToolsBinary =
        directory.childDirectory('cmdline-tools').childFile(binaryName);
    if (cmdlineToolsBinary.existsSync()) {
      return cmdlineToolsBinary.path;
    }
    final File platformToolBinary =
        directory.childDirectory('platform-tools').childFile(binaryName);
  ...
  String? getAvdManagerPath() => getCmdlineToolsPath(
      globals.platform.isWindows ? 'avdmanager.bat' : 'avdmanager');
  ...
  void reinitialize({FileSystem? fileSystem}) {
    ...
    _sdkVersions = _platforms
        .map<AndroidSdkVersion?>((Directory platformDir) {
          ...
          try {
            final Match? numberedVersion =
                _numberedAndroidPlatformRe.firstMatch(platformName);
  ...
  String? get sdkManagerVersion {
    if (sdkManagerPath == null ||
        !globals.processManager.canRun(sdkManagerPath)) {
      throwToolExit(
