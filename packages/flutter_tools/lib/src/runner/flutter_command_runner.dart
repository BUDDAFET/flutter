...
class FlutterCommandRunner extends CommandRunner<void> {
  ...
  Future<void> run(Iterable<String> args) {
    ...
    _machineFlagPresentInAnyCliArg =
        args.contains('--${FlutterGlobalOptions.kMachineFlag}');
  ...
  Future<bool> _shouldCheckForUpdates(
    ...
    final bool isShellCompletionCommand = !globals.stdio.hasTerminal &&
  ...
  Future<void> runCommand(ArgResults topLevelResults) async {
    ...
    if (topLevelResults.wasParsed(FlutterGlobalOptions.kWrapColumnOption)) {
      try {
        ...
        if (wrapColumn < 0) {
          throwToolExit(globals.userMessages.runnerWrapColumnInvalid(
              topLevelResults[FlutterGlobalOptions.kWrapColumnOption]));
        }
      } on FormatException {
        throwToolExit(globals.userMessages.runnerWrapColumnParseError(
            topLevelResults[FlutterGlobalOptions.kWrapColumnOption]));
      }
    }
    ...
    final EngineBuildPaths? engineBuildPaths =
      ...
      localHostEngine:
          topLevelResults[FlutterGlobalOptions.kLocalEngineHostOption]
              as String?,
      localWebSdk:
    ...
    await context.run<void>(
      ...
      body: () async {
        ...
        if (await _shouldCheckForUpdates(topLevelResults,
            topLevelMachineFlag: machineFlag)) {
          await globals.flutterVersion.checkFlutterVersionFreshness();
        }
