...
class AssembleCommand extends FlutterCommand {
  ...
  Map<String, String> _parseDefines(List<String> values) {
    ...
    final Map<String, Object?> defineConfigJsonMap =
        extractDartDefineConfigJsonMap();
    final List<String> dartDefines =
        extractDartDefines(defineConfigJsonMap: defineConfigJsonMap);
  ...
  Future<FlutterCommandResult> runCommand() async {
      ...
      for (final ExceptionMeasurement measurement in result.exceptions.values) {
        if (measurement.fatal || globals.logger.isVerbose) {
          globals.printError(
            ...
            stackTrace:
                globals.logger.isVerbose ? measurement.stackTrace : null,
          );
        }
      }
      throwToolExit('');
    ...
    if (argumentResults.wasParsed('depfile')) {
      ...
      _environment.depFileService
          .writeToFile(depfile, globals.fs.file(depfileFile));
    }
