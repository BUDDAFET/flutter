...
''';
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (devices.isEmpty) {
      logger.printStatus(
          'No custom devices found in "${customDevicesConfig.configPath}"');
    } else {
      logger.printStatus(
          'List of custom devices in "${customDevicesConfig.configPath}":');
...
''';
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    ErrorHandlingFileSystem.deleteIfExists(
        fileSystem!.file(customDevicesConfig.configPath));
...
class CustomDevicesAddCommand extends CustomDevicesCommandBase {
  ...
  Future<bool> _checkConfigWithLogging(final CustomDeviceConfig config) async {
    final CustomDevice device = CustomDevice(
        config: config, logger: logger, processManager: _processManager);
    ...
    try {
      final bool ok =
          await device.tryInstall(localPath: temp.path, appName: temp.basename);
      if (!ok) {
    ...
    if (config.usesPortForwarding) {
      ...
      try {
        ...
        final ForwardedPort? forwardedPort =
            await portForwarder.tryForward(port, port);
  ...
  Future<FlutterCommandResult> runInteractivelySsh() async {
    ...
    final String remoteRunDebugCommand = (await askForString('run command',
        description:
