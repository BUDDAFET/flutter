...
class BuildIOSCommand extends _BuildIOSSubCommand {
  ...
  @override
  EnvironmentType get environmentType => boolArg('simulator')
...
class _ImageAssetFileKey {
  ...
  int? get pixelSize => size == null
      ? null
      : (size! * scale).toInt(); // pixel size must be an int.
}
...
class BuildIOSArchiveCommand extends _BuildIOSSubCommand {
  ...
  final String description =
      'Build an iOS archive bundle and IPA for distribution.';
  ...
  Map<_ImageAssetFileKey, String> _parseImageAssetContentsJson(
    ...
    final Directory contentsJsonDirectory =
        globals.fs.directory(contentsJsonDirName);
    if (!contentsJsonDirectory.existsSync()) {
      return <_ImageAssetFileKey, String>{};
    }
    final File contentsJsonFile =
        contentsJsonDirectory.childFile('Contents.json');
    final Map<String, dynamic> contents =
        json.decode(contentsJsonFile.readAsStringSync())
                as Map<String, dynamic>? ??
            <String, dynamic>{};
    final List<dynamic> images =
        contents['images'] as List<dynamic>? ?? <dynamic>[];
    ...
    final Map<_ImageAssetFileKey, String> iconInfo =
        <_ImageAssetFileKey, String>{};
  ...
  bool _isAssetStillUsingTemplateFiles({
    ...
    return projectImageInfoMap.entries
        .any((MapEntry<_ImageAssetFileKey, String> entry) {
      final String projectFileName = entry.value;
      final String? templateFileName = templateImageInfoMap[entry.key];
      if (templateFileName == null) {
        return false;
      }
      final File projectFile = globals.fs
          .file(globals.fs.path.join(projectImageDirName, projectFileName));
      final File templateFile = globals.fs
          .file(globals.fs.path.join(templateImageDirName, templateFileName));
    ...
    return imageInfoMap.entries
        .where((MapEntry<_ImageAssetFileKey, String> entry) {
          ...
          final File imageFile =
              globals.fs.file(globals.fs.path.join(imageDirName, fileName));
          ...
          final ByteData imageData =
              imageFile.readAsBytesSync().buffer.asByteData();
  ...
  ValidationResult? _createValidationResult(
      String title, List<ValidationMessage> messages) {
    if (messages.isEmpty) {
      return null;
    }
    final bool anyInvalid = messages.any((ValidationMessage message) =>
        message.type != ValidationMessageType.information);
  ...
  ValidationMessage _createValidationMessage({
    ...
    return isValid
        ...
        : ValidationMessage.hint(message);
  }

  Future<List<ValidationMessage>> _validateIconAssetsAfterArchive() async {
    final BuildableIOSApp app = await buildableIOSApp;

    final Map<_ImageAssetFileKey, String> templateInfoMap =
        _parseImageAssetContentsJson(app.templateAppIconDirNameForContentsJson,
            requiresSize: true);
    final Map<_ImageAssetFileKey, String> projectInfoMap =
        _parseImageAssetContentsJson(app.projectAppIconDirName,
            requiresSize: true);
    ...
    final List<String> filesWithWrongSize = _imageFilesWithWrongSize(
        imageInfoMap: projectInfoMap, imageDirName: app.projectAppIconDirName);

    if (filesWithWrongSize.isNotEmpty) {
      validationMessages.add(_createValidationMessage(
        isValid: false,
        message:
            'App icon is using the incorrect size (e.g. ${filesWithWrongSize.first}).',
      ));
    }
    return validationMessages;
  }

  Future<List<ValidationMessage>>
      _validateLaunchImageAssetsAfterArchive() async {
    final BuildableIOSApp app = await buildableIOSApp;
    ...
    final Map<_ImageAssetFileKey, String> templateInfoMap =
        _parseImageAssetContentsJson(
            ...
            requiresSize: false);
    final Map<_ImageAssetFileKey, String> projectInfoMap =
        _parseImageAssetContentsJson(app.projectLaunchImageDirName,
            requiresSize: false);
  ...
  Future<List<ValidationMessage>>
      _validateXcodeBuildSettingsAfterArchive() async {
    final BuildableIOSApp app = await buildableIOSApp;
    ...
    xcodeProjectSettingsMap['Version Number'] = globals.plistParser
        .getValueFromFile<String>(
            plistPath, PlistParser.kCFBundleShortVersionStringKey);
    xcodeProjectSettingsMap['Build Number'] = globals.plistParser
        .getValueFromFile<String>(plistPath, PlistParser.kCFBundleVersionKey);
    xcodeProjectSettingsMap['Display Name'] = globals.plistParser
            .getValueFromFile<String>(
                plistPath, PlistParser.kCFBundleDisplayNameKey) ??
        globals.plistParser
            .getValueFromFile<String>(plistPath, PlistParser.kCFBundleNameKey);
    xcodeProjectSettingsMap['Deployment Target'] = globals.plistParser
        .getValueFromFile<String>(plistPath, PlistParser.kMinimumOSVersionKey);
    xcodeProjectSettingsMap['Bundle Identifier'] = globals.plistParser
        .getValueFromFile<String>(
            plistPath, PlistParser.kCFBundleIdentifierKey);
    ...
    final bool hasMissingSettings = xcodeProjectSettingsMap.values
    ...
    final bool usesDefaultBundleIdentifier =
        xcodeProjectSettingsMap['Bundle Identifier']
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    validationResults.add(_createValidationResult(
      ...
      await _validateIconAssetsAfterArchive() +
          await _validateLaunchImageAssetsAfterArchive(),
    ));

    for (final ValidationResult result
        in validationResults.whereType<ValidationResult>()) {
    ...
    final String absoluteOutputPath =
        globals.fs.path.absolute(relativeOutputPath);
    final String absoluteArchivePath =
    ...
    String? exportMethod = exportOptions != null
        ...
        : null;
    exportMethod ??=
        _getVersionAppropriateExportMethod(stringArg('export-method')!);
    final bool isAppStoreUpload =
        exportMethod == 'app-store' || exportMethod == 'app-store-connect';
    File? generatedExportPlist;
    try {
      final String exportMethodDisplayName =
          isAppStoreUpload ? 'App Store' : exportMethod;
      status = globals.logger
          .startProgress('Building $exportMethodDisplayName IPA...');
      if (exportOptions == null) {
    ...
    if (result.exitCode != 0) {
      ...
      globals.printError(
          'Try distributing the app in Xcode: "open $absoluteArchivePath"');
...
abstract class _BuildIOSSubCommand extends BuildSubCommand {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final String logTarget =
        environmentType == EnvironmentType.simulator ? 'simulator' : 'device';
    final String typeName =
        globals.artifacts!.getEngineType(TargetPlatform.ios, buildInfo.mode);
    ...
    if (!result.success) {
      ...
      throwToolExit(
          'Encountered error while $presentParticiple for $logTarget.');
    }
    ...
    if (buildInfo.codeSizeDirectory != null) {
      ...
      final Directory outputAppDirectoryCandidate =
          _outputAppDirectory(resultOutput);
