...
class WebTestCompiler {
  ...
  Future<File> _generateTestEntrypoint({
    ...
    final List<WebTestInfo> testInfos = testFiles.map((String testFilePath) {
      ...
      final File? testConfigFile =
          findTestConfigFile(_fileSystem.file(testFilePath), _logger);
      String? testConfigPath;
      if (testConfigFile != null) {
        testConfigPath = _fileSystem.path
            .split(_fileSystem.path.relative(testConfigFile.path,
                from: projectDirectory.childDirectory('test').path))
            .join('/');
      }
      return (
    ...
    return _fileSystem
        .file(_fileSystem.path.join(outputDirectory.path, 'main.dart'))
      ..createSync(recursive: true)
      ..writeAsStringSync(generateTestEntrypoint(
          testInfos: testInfos, languageVersion: languageVersion));
  }
  ...
  Future<WebMemoryFS> _compileJS({
    ...
    switch (buildInfo.nullSafetyMode) {
      ...
      case NullSafetyMode.sound:
        languageVersion =
            currentLanguageVersion(_fileSystem, Cache.flutterRoot!);
    ...
    final List<String> dartDefines =
        webRenderer.updateDartDefines(buildInfo.dartDefines);
  ...
  Future<WebMemoryFS> _compileWasm({
    ...
    final String platformFilePath =
        _fileSystem.path.join(platformBinariesPath, 'dart2wasm_platform.dill');
    final List<String> dartDefines =
        webRenderer.updateDartDefines(buildInfo.dartDefines);
