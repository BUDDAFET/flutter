...
class FlutterTesterTestDevice extends TestDevice {
    ...
    _pipeStandardStreamsToConsole(
      ...
      reportVmServiceUri: (Uri detectedUri) async {
        ...
        if (debuggingOptions.startPaused && !machine!) {
          logger.printStatus(
              'The Dart VM service is listening on $forwardingUri');
  ...
  Future<void> _startDevTools(
      Uri forwardingUri, DartDevelopmentService? dds) async {
    ...
    logger.printTrace(
        'test $id: DevTools is being served at ${devToolsServerAddress.uri}');
        ...
        (String line) async {
          ...
          if (match != null) {
            try {
              ...
              logger.printError(
                  'Could not parse shell VM Service port message: $error');
            }
          } else {
...
String _getExitCodeMessage(int exitCode) {
  return switch (exitCode) {
    ...
    -0x0f =>
      'Shell subprocess crashed with SIGTERM ($exitCode).', // ProcessSignal.SIGTERM
    -0x0b =>
      'Shell subprocess crashed with segmentation fault.', // ProcessSignal.SIGSEGV
    -0x06 =>
      'Shell subprocess crashed with SIGABRT ($exitCode).', // ProcessSignal.SIGABRT
    -0x02 =>
      'Shell subprocess terminated by ^C (SIGINT, $exitCode).', // ProcessSignal.SIGINT
