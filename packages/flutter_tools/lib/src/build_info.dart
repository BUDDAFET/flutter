...
class BuildInfo {
  ...
  DdcModuleFormat? get ddcModuleFormat =>
      _ddcModuleFormatFromFrontEndArgs(extraFrontEndOptions);
...
String? validatedBuildNumberForPlatform(
    TargetPlatform targetPlatform, String? buildNumber, Logger logger) {
  if (buildNumber == null) {
    return null;
  }
...
String? validatedBuildNameForPlatform(
    TargetPlatform targetPlatform, String? buildName, Logger logger) {
  if (buildName == null) {
    return null;
  }
  if (targetPlatform == TargetPlatform.ios ||
...
String getNameForTargetPlatform(TargetPlatform platform,
  ...
  return switch (platform) {
    ...
    TargetPlatform.darwin when darwinArch != null =>
      'darwin-${darwinArch.name}',
...
String getBuildDirectory([Config? config, FileSystem? fileSystem]) {
  ...
  final String buildDir =
      localConfig.getValue('build-dir') as String? ?? 'build';
...
String getAssetBuildDirectory([Config? config, FileSystem? fileSystem]) {
  return (fileSystem ?? globals.fs)
      .path
      .join(getBuildDirectory(config, fileSystem), 'flutter_assets');
}
...
final Converter<String, String> _defineEncoder =
    utf8.encoder.fuse(base64.encoder);
final Converter<String, String> _defineDecoder =
    base64.decoder.fuse(utf8.decoder);
...
List<String> decodeCommaSeparated(
    Map<String, String> environmentDefines, String key) {
  if (!environmentDefines.containsKey(key) ||
      environmentDefines[key]!.isEmpty) {
    return <String>[];
  }
...
// TODO(markzipan): delete this when DDC's AMD module system is deprecated, https://github.com/flutter/flutter/issues/142060.
DdcModuleFormat? _ddcModuleFormatFromFrontEndArgs(
    List<String>? extraFrontEndArgs) {
  ...
  for (final String flag in extraFrontEndArgs) {
    if (flag.startsWith(ddcModuleFormatString)) {
      final String moduleFormatString =
          flag.substring(ddcModuleFormatString.length, flag.length);
