...
void main() {
  group('Apk with partial Android SDK works', () {
    ...
    setUp(() async {
      ...
      fs
          .file(project.android.hostAppGradleRoot
              .childFile(
    ...
    testUsingContext('correct debug filename in module projects', () async {
      ...
      fakeProcessManager.addCommand(FakeCommand(command: <String>[
        ...
        'dump',
        'xmltree',
        fs.path.join('module_project', 'build', 'host', 'outputs', 'apk',
            'debug', 'app-debug.apk'),
        'AndroidManifest.xml',
      ], stdout: _aaptDataWithDefaultEnabledAndMainLauncherActivity));
      ...
      project.android.hostAppGradleRoot
          ...
          .createSync(recursive: true);
      final File appGradle = project.android.hostAppGradleRoot
          .childFile(fs.path.join('app', 'build.gradle'));
      ...
      final AndroidApk? androidApk = await AndroidApk.fromAndroidProject(
        ...
        processUtils:
            ProcessUtils(processManager: fakeProcessManager, logger: logger),
        logger: logger,
        fileSystem: fs,
    ...
    testUsingContext(
      ...
      expect(
          (applicationPackage as PrebuiltApplicationPackage)
              .applicationPackage
              .path,
          apkFile.path);
    ...
    testUsingContext('Licenses available, build tools not, apk exists',
        () async {
      ...
      final Directory gradleWrapperDir =
          cache.getArtifactDirectory('gradle_wrapper');

      gradleWrapperDir.fileSystem
          .directory(gradleWrapperDir
    ...
    testWithoutContext('returns null when failed to extract manifest',
        () async {
  ...
  group('ApkManifestData', () {
    testWithoutContext(
      ...
      expect(data.launchableActivityName,
    ...
    testWithoutContext('Parses manifest with missing application tag',
        () async {
      final ApkManifestData? data = ApkManifestData.parseFromXmlDump(
  ...
  group('PrebuiltIOSApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      final PrebuiltIOSApp? iosApp =
          IOSApp.fromPrebuiltApp(globals.fs.file('not_existing.ipa'))
              as PrebuiltIOSApp?;
    ...
    testUsingContext('Error on non-app-bundle folder', () {
      ...
      final PrebuiltIOSApp? iosApp =
          IOSApp.fromPrebuiltApp(globals.fs.file('regular_folder'))
              as PrebuiltIOSApp?;
      expect(iosApp, isNull);
    ...
    testUsingContext('Error on no info.plist', () {
      ...
      final PrebuiltIOSApp? iosApp =
          IOSApp.fromPrebuiltApp(globals.fs.file('bundle.app'))
              as PrebuiltIOSApp?;
      expect(iosApp, isNull);
      expect(
        testLogger.errorText,
    ...
    testUsingContext('Success with app bundle', () {
      ...
      expect(iosApp.applicationPackage.path,
          globals.fs.directory('bundle.app').path);
    }, overrides: overrides);
    ...
    testUsingContext('Bad ipa zip-file, two app bundles', () {
      ...
      os.onUnzip = (File zipFile, Directory targetDirectory) {
        ...
        final String bundlePath1 = globals.fs.path
            .join(targetDirectory.path, 'Payload', 'bundle1.app');
        final String bundlePath2 = globals.fs.path
            .join(targetDirectory.path, 'Payload', 'bundle2.app');
    ...
    testUsingContext('Success with ipa', () {
      ...
      os.onUnzip = (File zipFile, Directory targetDirectory) {
        ...
        final Directory bundleAppDir = globals.fs.directory(globals.fs.path
            .join(targetDirectory.path, 'Payload', 'bundle.app'));
    ...
    testUsingContext('returns null when there is no ios or .ios directory',
        () async {
      ...
      final BuildableIOSApp? iosApp = await IOSApp.fromIosProject(
          ...
          null) as BuildableIOSApp?;

      expect(iosApp, null);
    }, overrides: overrides);

    testUsingContext('returns null when there is no Runner.xcodeproj',
        () async {
      globals.fs.file('pubspec.yaml').createSync();
      globals.fs.file('.packages').createSync();
    ...
    testUsingContext('returns null when there with no product identifier',
        () async {
      globals.fs.file('pubspec.yaml').createSync();
      globals.fs.file('.packages').createSync();
    ...
    testUsingContext('returns project app icon dirname', () async {
      final BuildableIOSApp iosApp = BuildableIOSApp(
        IosProject.fromFlutter(
            FlutterProject.fromDirectory(globals.fs.currentDirectory)),
        'com.foo.bar',
        'Runner',
      );
      ...
      expect(iosApp.projectAppIconDirName,
          globals.fs.path.join('ios', iconDirSuffix));
    }, overrides: overrides);

    testUsingContext('returns template app icon dirname for Contents.json',
        () async {
      final BuildableIOSApp iosApp = BuildableIOSApp(
        IosProject.fromFlutter(
            FlutterProject.fromDirectory(globals.fs.currentDirectory)),
    ...
    testUsingContext('returns template app icon dirname for images', () async {
      ...
      final String packageConfigPath =
          globals.fs.path.join(toolsDir, '.dart_tool', 'package_config.json');
...
''');
    ...
    testUsingContext('returns project launch image dirname', () async {
      ...
      expect(iosApp.projectLaunchImageDirName,
          globals.fs.path.join('ios', launchImageDirSuffix));
    }, overrides: overrides);

    testUsingContext('returns template launch image dirname for Contents.json',
        () async {
      final BuildableIOSApp iosApp = BuildableIOSApp(
        IosProject.fromFlutter(
            FlutterProject.fromDirectory(globals.fs.currentDirectory)),
    ...
    testUsingContext('returns template launch image dirname for images',
        () async {
      final String toolsDir = globals.fs.path.join(
        Cache.flutterRoot!,
        'packages',
  ...
  group('FuchsiaApp', () {
    ...
    testUsingContext('Error on non-existing file', () {
      final PrebuiltFuchsiaApp? fuchsiaApp =
          FuchsiaApp.fromPrebuiltApp(globals.fs.file('not_existing.far'))
              as PrebuiltFuchsiaApp?;
    ...
    testUsingContext('Error on non-far file', () {
      globals.fs.directory('regular_folder').createSync();
      final PrebuiltFuchsiaApp? fuchsiaApp =
          FuchsiaApp.fromPrebuiltApp(globals.fs.file('regular_folder'))
              as PrebuiltFuchsiaApp?;
      expect(fuchsiaApp, isNull);
      expect(
        testLogger.errorText,
    ...
    testUsingContext('Success with far file', () {
      ...
      final PrebuiltFuchsiaApp fuchsiaApp =
          FuchsiaApp.fromPrebuiltApp(globals.fs.file('bundle.far'))!
              as PrebuiltFuchsiaApp;
      expect(testLogger.errorText, isEmpty);
      expect(fuchsiaApp.id, 'bundle.far');
      expect(fuchsiaApp.applicationPackage.path,
          globals.fs.file('bundle.far').path);
    }, overrides: overrides);

    testUsingContext('returns null when there is no fuchsia', () async {
      globals.fs.file('pubspec.yaml').createSync();
      globals.fs.file('.packages').createSync();
      final BuildableFuchsiaApp? fuchsiaApp = FuchsiaApp.fromFuchsiaProject(
              FlutterProject.fromDirectory(globals.fs.currentDirectory).fuchsia)
          as BuildableFuchsiaApp?;
