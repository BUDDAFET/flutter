...
''');
  ...
  group('Version', () {
    testWithoutContext('can parse and compare', () {
      ...
      expect(const Version.withText(1, 2, 3, 'versionText').toString(),
          'versionText');
  ...
  group('text wrapping', () {
    ...
    final String longAnsiLineWithNewlines =
        ...
        '${'0123456789' * 3}'
        '${AnsiTerminal.green}0123456789${AnsiTerminal.resetAll}';
    const String onlyAnsiSequences =
        '${AnsiTerminal.red}${AnsiTerminal.resetAll}';
    ...
    testWithoutContext('does not wrap by default in tests', () {
      expect(wrapText(longLine, columnWidth: 80, shouldWrap: true),
          equals(longLine));
    });

    testWithoutContext('can override wrap preference if preference is off', () {
      expect(wrapText(longLine, columnWidth: lineLength, shouldWrap: true),
          equals('''
...
wrapped.'''));
    ...
    testWithoutContext('can override wrap preference if preference is on', () {
      expect(wrapText(longLine, shouldWrap: false, columnWidth: 80),
          equals(longLine));
    });

    testWithoutContext('does not wrap at all if not told to wrap', () {
      expect(wrapText(longLine, columnWidth: 80, shouldWrap: false),
          equals(longLine));
    });

    testWithoutContext('does not wrap short lines.', () {
      expect(wrapText(shortLine, columnWidth: lineLength, shouldWrap: true),
          equals(shortLine));
    });
...
wrapped.'''));
    ...
    testWithoutContext(
        'able to handle dynamically changing terminal column size', () {
...
0123456789'''));
    ...
    testWithoutContext('refuses to wrap to a column smaller than 10 characters',
        () {
      expect(
          wrapText('$longLine ${'0123456789' * 4}',
              columnWidth: 1, shouldWrap: true),
          equals('''
This is a
long line
that needs
...
0123456789'''));
    ...
    testWithoutContext(
        'preserves indentation and stripping trailing whitespace', () {
      expect(
          wrapText('$indentedLongLine   ',
              columnWidth: lineLength, shouldWrap: true),
          equals('''
    This is an indented long line that
    needs to be wrapped and indentation
    preserved.'''));
...
0123456789'''));
    ...
    testWithoutContext('wraps text with ANSI sequences embedded', () {
      expect(
          wrapText(longAnsiLineWithNewlines,
              columnWidth: lineLength, shouldWrap: true),
          equals('''
${AnsiTerminal.red}This${AnsiTerminal.resetAll} is a long line with newlines that
needs to be wrapped.
...
${AnsiTerminal.green}0123456789${AnsiTerminal.resetAll}'''));
    ...
    testWithoutContext('preserves indentation in the presence of newlines', () {
      expect(
          wrapText(indentedLongLineWithNewlines,
              columnWidth: lineLength, shouldWrap: true),
          equals('''
...
'''));
    ...
    testWithoutContext(
        'honors hangingIndent parameter on already indented line.', () {
      expect(
          wrapText(indentedLongLine,
              columnWidth: lineLength, hangingIndent: 6, shouldWrap: true),
          equals('''
    ...
    testWithoutContext(
        'honors hangingIndent and indent parameters at the same time.', () {
      expect(
          wrapText(indentedLongLine,
    ...
    testWithoutContext('honors indent parameter on already indented line.', () {
      expect(
          wrapText(indentedLongLine,
              columnWidth: lineLength, indent: 6, shouldWrap: true),
          equals('''
          This is an indented long line
...
needs to be wrapped.
  ...
  01234567890123456789012345678901234567
    ...
    testWithoutContext('', () {
      expect(
        wrapText('${' ' * 7}abc def ghi',
            columnWidth: 20, hangingIndent: 5, indent: 3, shouldWrap: true),
        equals('          abc def\n'
            '          ghi'),
      );
      expect(
        wrapText('abc def ghi',
            columnWidth: 0, hangingIndent: 5, shouldWrap: true),
        equals('abc def\n'
            'ghi'),
      );
      expect(
      ...
      expect(
        wrapText('abc def ghi',
            columnWidth: kMinColumnWidth - 2, indent: 5, shouldWrap: true),
        equals('abc def\n'
            'ghi'),
      );
      expect(
        wrapText('     abc def ghi',
            columnWidth: kMinColumnWidth - 2, shouldWrap: true),
        equals('abc def\n'
            'ghi'),
      );
      expect(
        wrapText('abc def ghi jkl',
            columnWidth: kMinColumnWidth + 2,
            hangingIndent: 5,
            shouldWrap: true),
        equals('abc def ghi\n'
  ...
  testWithoutContext('getSizeAsMB', () async {
    ...
    expect(
        getSizeAsPlatformMB(10 * 1000 * 1000,
    ...
    expect(
        getSizeAsPlatformMB(10 * 1000 * 1000,
            platform: FakePlatform(operatingSystem: 'web')),
        '10.0MB');
  });
}
