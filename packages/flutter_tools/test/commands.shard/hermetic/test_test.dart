...
final String _flutterToolsPackageConfigContents = json.encode(<String, Object>{
  ...
  'packages': <Map<String, Object>>[
    ...
    <String, String>{
      'name': 'test',
      'rootUri':
          'file:///path/to/pubcache/.pub-cache/hosted/pub.dev/test-1.24.9',
      'packageUri': 'lib/',
      'languageVersion': '3.0'
    },
    <String, String>{
      'name': 'test_api',
      'rootUri':
          'file:///path/to/pubcache/.pub-cache/hosted/pub.dev/test_api-0.6.1',
      'packageUri': 'lib/',
      'languageVersion': '3.0'
    },
    <String, String>{
      'name': 'test_core',
      'rootUri':
          'file:///path/to/pubcache/.pub-cache/hosted/pub.dev/test_core-0.5.9',
      'packageUri': 'lib/',
      'languageVersion': '3.0'
    },
  ],
...
void main() {
  ...
  setUp(() {
    ...
    final File flutterToolsPackageConfigFile = fs
        .directory(
          fs.path.join(getFlutterRoot(), 'packages', 'flutter_tools'),
        )
...
dev_dependencies:
  ...
  group('--reporter/-r', () {
    ...
    Future<void> expectPassesReporter(String value) async {
      ...
      await commandRunner
          .run(<String>['test', '--no-pub', '--reporter', value]);
      expect(passedReporter(fakePackageTest.lastArgs!), equals(value));
  ...
  testUsingContext(
    ...
    final FakeFlutterTestRunner testRunner =
        FakeFlutterTestRunner(0, null, fakeVmServiceHost);
  ...
  group('Pipes to package:test', () {
    ...
    testUsingContext('passes various CLI options through to package:test',
        () async {
  ...
  testUsingContext(
    ...
    await asyncGuard<void>(
      ...
      // [_flutterToolsPackageConfigContents].
      expect(
        isolateSpawningTesterPackageConfigFile
            .readAsStringSync()
            .contains('"name": "integration_test"'),
        true,
      );
      expect(
        isolateSpawningTesterPackageConfigFile
      ...
      expect(
        isolateSpawningTesterPackageConfigFile
            .readAsStringSync()
            .contains('"name": "test_api"'),
        true,
      );
      expect(
        isolateSpawningTesterPackageConfigFile
            .readAsStringSync()
            .contains('"name": "test_core"'),
        true,
      );
    });
  ...
  group('File Reporter', () {
    ...
    testUsingContext('when set --file-reporter value is passed on', () async {
      ...
      await commandRunner.run(
          const <String>['test', '--no-pub', '--file-reporter=json:out.jsonl']);
    ...
    testUsingContext('Enables Impeller', () async {
      ...
      expect(testRunner.lastDebuggingOptionsValue.enableImpeller,
    ...
    testUsingContext('Passes web renderer into debugging options', () async {
      ...
      expect(testRunner.lastDebuggingOptionsValue.webRenderer,
    ...
    testUsingContext('Web renderer defaults to Skwasm when using wasm',
        () async {
      final FakeFlutterTestRunner testRunner = FakeFlutterTestRunner(0);
...
class FakeFlutterTestRunner implements FlutterTestRunner {
  FakeFlutterTestRunner(this.exitCode,
      [this.leastRunTime, this.fakeVmServiceHost]);
