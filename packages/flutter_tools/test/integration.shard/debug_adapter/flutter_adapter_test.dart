...
The relevant error-causing widget was:
  App'''),
    ...
    group('can step', () {
      test('into SDK sources mapped to local files when debugSdkLibraries=true',
          () async {
        final BasicProject project = BasicProject();
        await project.setUpIn(tempDir);

        final String breakpointFilePath =
            globals.fs.path.join(project.dir.path, 'lib', 'main.dart');
        final int breakpointLine = project.topLevelFunctionBreakpointLine;
        final String expectedPrintLibraryPath = globals.fs.path
            .join('pkg', 'sky_engine', 'lib', 'core', 'print.dart');
        ...
        int stoppedThreadId = (await dap.client.stoppedEvents
        ...
        // correctly.
        final StackFrame topFrame = (await dap.client
                .getValidStack(stoppedThreadId, startFrame: 0, numFrames: 1))
            .stackFrames
            .single;
