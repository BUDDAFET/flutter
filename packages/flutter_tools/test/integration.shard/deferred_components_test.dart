...
void main() {
  ...
  testWithoutContext('simple build appbundle android-arm64 target succeeds',
      () async {
    final DeferredComponentsProject project =
        DeferredComponentsProject(BasicDeferredComponentsConfig());
    ...
    expect(
        result, const ProcessResultMatcher(stdoutPattern: 'app-release.aab'));
    expect(result.stdout.toString(),
        contains('Deferred components prebuild validation passed.'));
    expect(result.stdout.toString(),
    ...
    final File outputFile =
        fileSystem.file(fileSystem.path.join(tempDir.path, outputFilePath));
    expect(outputFile, exists);

    final Archive archive =
        ZipDecoder().decodeBytes(outputFile.readAsBytesSync());
    ...
    expect(
        archive.findFile('component1/lib/arm64-v8a/libapp.so-2.part.so') !=
            null,
        true);
    ...
    expect(
        ...
        true);
    expect(
        archive.findFile('base/assets/flutter_assets/test_assets/asset1.txt') !=
            null,
        true);
  });
  ...
  testWithoutContext('simple build appbundle all targets succeeds', () async {
    ...
    expect(
        archive.findFile('base/lib/armeabi-v7a/libflutter.so') != null, true);
    expect(
        archive.findFile('component1/lib/armeabi-v7a/libapp.so-2.part.so') !=
            null,
        true);
    ...
    expect(
        archive.findFile('component1/lib/x86_64/libapp.so-2.part.so') != null,
        true);
  ...
  testWithoutContext('simple build appbundle no-deferred-components succeeds',
      () async {
    final DeferredComponentsProject project =
        DeferredComponentsProject(BasicDeferredComponentsConfig());
    await project.setUpIn(tempDir);
    ...
    expect(
        ...
        false);
    expect(
        archive.findFile('base/assets/flutter_assets/test_assets/asset2.txt') !=
            null,
        true);
    expect(
        archive.findFile('base/assets/flutter_assets/test_assets/asset1.txt') !=
  ...
  testWithoutContext(
      ...
      () async {
    final DeferredComponentsProject project =
        DeferredComponentsProject(MismatchedGoldenDeferredComponentsConfig());
    await project.setUpIn(tempDir);
    final String flutterBin =
        fileSystem.path.join(getFlutterRoot(), 'bin', 'flutter');
    final ProcessResult result = await processManager.run(<String>[
    ...
    expect(result.stdout.toString(),
        isNot(contains('Deferred components gen_snapshot validation passed.')));
    expect(result.stdout.toString(),
        isNot(contains('New loading units were found:')));
    expect(result.stdout.toString(),
  ...
  testWithoutContext(
      'simple build appbundle missing android dynamic feature module fails',
      () async {
    ...
    expect(
        ...
        const ProcessResultMatcher(
            exitCode: 1, stdoutPattern: 'Newly generated android files:'));
  ...
  testWithoutContext('simple build appbundle missing golden fails', () async {
    final DeferredComponentsProject project =
        DeferredComponentsProject(NoGoldenDeferredComponentsConfig());
    await project.setUpIn(tempDir);
    final String flutterBin =
        fileSystem.path.join(getFlutterRoot(), 'bin', 'flutter');
    final ProcessResult result = await processManager.run(<String>[
    ...
    expect(result.stdout.toString(),
        contains('- package:test/deferred_library.dart'));

    expect(result.stdout.toString(),
        isNot(contains('Previously existing loading units no longer exist:')));
  });

  testWithoutContext('simple build appbundle mismatched golden fails',
      () async {
    final DeferredComponentsProject project =
        DeferredComponentsProject(MismatchedGoldenDeferredComponentsConfig());
    await project.setUpIn(tempDir);
    ...
    expect(result.stdout.toString(),
        contains('- package:test/invalid_lib_name.dart'));
