...
void main() {
  group('TestFlutterView', () {
    ...
    testWidgets(
      ...
      expect(
          tester.view.display.devicePixelRatio, tester.view.devicePixelRatio);
    });
    ...
    testWidgets('faking physicalSize fakes physicalConstraints',
        (WidgetTester tester) async {
    ...
    testWidgets('resetting physicalSize resets physicalConstraints',
        (WidgetTester tester) async {
      const Size fakeSize = Size(50, 50);
    ...
    testWidgets('can fake physicalConstraints', (WidgetTester tester) async {
      verifyPropertyFaked<ViewConstraints>(
        tester: tester,
        realValue: trueImplicitView().physicalConstraints,
        fakeValue: const ViewConstraints(
            minWidth: 1, maxWidth: 2, minHeight: 3, maxHeight: 4),
        propertyRetriever: () => boundImplicitView().physicalConstraints,
    ...
    testWidgets('can clear out fake properties all at once',
        (WidgetTester tester) async {
    ...
    testWidgets('render is passed through to backing FlutterView',
        (WidgetTester tester) async {
...
Matcher matchesSnapshot(FlutterViewSnapshot expected) =>
...
class _FlutterViewSnapshotMatcher extends Matcher {
  ...
  Description describeMismatch(dynamic item, Description mismatchDescription,
    ...
    assert(item is FlutterViewSnapshot,
    ...
    if (!actual.displayFeatures.equals(expected.displayFeatures)) {
      ...
      mismatchDescription.addAll(
          'Expected: [', ',', ']', expected.displayFeatures);
    }
    ...
    if (!paddingMatcher.matches(actual.padding, matchState)) {
      ...
      paddingMatcher.describeMismatch(
          actual.padding, mismatchDescription, matchState, verbose);
    }
    ...
    final Matcher systemGestureInsetsMatcher =
        matchesViewPadding(expected.systemGestureInsets);
    if (!systemGestureInsetsMatcher.matches(
        actual.systemGestureInsets, matchState)) {
      mismatchDescription.add(
      ...
      systemGestureInsetsMatcher.describeMismatch(
          actual.systemGestureInsets, mismatchDescription, matchState, verbose);
    }
    ...
    if (!viewInsetsMatcher.matches(actual.viewInsets, matchState)) {
      ...
      viewInsetsMatcher.describeMismatch(
          actual.viewInsets, mismatchDescription, matchState, verbose);
    }
    ...
    if (!viewPaddingMatcher.matches(actual.viewPadding, matchState)) {
      ...
      viewPaddingMatcher.describeMismatch(
          actual.viewPadding, mismatchDescription, matchState, verbose);
    }
  ...
  bool matches(dynamic item, Map<dynamic, dynamic> matchState) {
    ...
    return actual.devicePixelRatio == expected.devicePixelRatio &&
        ...
        matchesViewPadding(expected.padding)
            .matches(actual.padding, matchState) &&
        actual.physicalSize == expected.physicalSize &&
        matchesViewPadding(expected.systemGestureInsets)
            .matches(actual.padding, matchState) &&
        actual.viewId == expected.viewId &&
        matchesViewPadding(expected.viewInsets)
            .matches(actual.viewInsets, matchState) &&
        matchesViewPadding(expected.viewPadding)
            .matches(actual.viewPadding, matchState);
  }
}
