...
void main() {
  ...
  test('analyze.dart - verifySpacesAfterFlowControlStatements', () async {
    final String result = await capture(
        () => verifySpacesAfterFlowControlStatements(testRootPath,
            minimumMatches: 2),
        shouldHaveErrors: true);
    final String lines = <String>[
    ...
    final String result = await capture(
        () => verifyNoBinaries(
              ...
              legacyBinaries: <Hash256>{
                const Hash256(0x39A050CD69434936, 0, 0, 0)
              },
            ),
  ...
  test('analyze.dart - verifyInternationalizations - comparison fails',
      () async {
    final String result = await capture(
    ...
    expect(
        result,
        contains(
            r'Did you forget to run gen_localizations.dart after updating a .arb file?'));
  });
  ...
  test('analyze.dart - verifyNoBinaries - negative', () async {
    await capture(() => verifyNoBinaries(
          testRootPath,
          legacyBinaries: <Hash256>{
            const Hash256(0xA8100AE6AA1940D0, 0xB663BB31CD466142,
                0xEBBDBD5187131B92, 0xD93818987832EB89), // sha256("\xff")
  ...
  test('analyze.dart - verifyNullInitializedDebugExpensiveFields', () async {
    final String result = await capture(
        () => verifyNullInitializedDebugExpensiveFields(
              testRootPath,
              minimumMatches: 1,
            ),
        shouldHaveErrors: true);
  ...
  test('analyze.dart - verifyMaterialFilesAreUpToDateWithTemplateFiles',
      () async {
    String result = await capture(
        () => verifyMaterialFilesAreUpToDateWithTemplateFiles(
              testGenDefaultsPath,
              dartPath,
            ),
        shouldHaveErrors: true);
